...
#define VARLENGTH_DATA                                                                             \
    (sizeof(NNUM) * 2) // Private data used at end of
...
class DBASE : private BLKIOBUF
{
    ...
    int _lookup(void **ppvFields, PALIST *ppl, void *pvFoundRec, NNUM *pnn, unsigned uiIdx,
                int fForward, int iLength, unsigned long *pulLength);
  ...
  public:
    ...
    int CPPEXPORT open(char *szNam, FIELD *pf, unsigned uiNumFields, unsigned new_file,
                       unsigned uiOrder = INDEX_ORDER, int fVarLength = FALSE);

    int CPPEXPORT insert(void *pvRecord, int iLength = 0);

    int CPPEXPORT lookup(void **ppvFields, PALIST *ppl, void *pvFoundRec, unsigned uiIdx = 0,
                         int iLength = 0, unsigned long *pulLength = 0);
