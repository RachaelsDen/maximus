...
VOCH VocOpen(char *name)
{
    ...
    {
        Bseek(handle->file_handle, (long)header.offset_of_data_block - sizeof(headers), SEEK_CUR);
    }
...
int VocRead(VOCH handle, blocks *block)
{
    ...
    if (handle->voice_info.voice_to_continue && handle->voice_info.remaining_length > 0)
    {
    ...
    case silence:
        ...
        block->block_length = 0;
        block->dat.silence_interval =
            ((duration)_VocReadThis2(handle) + 1.0) / _VocReadThisRate(handle);
        break;
    ...
    case text:
        ...
        if (Bread(handle->file_handle, (char *)&block->dat.text_string, block_length) == -1)
        {
            printf("Can't read text\n");
            return -1;
        }

        if (Bread(handle->file_handle, &null_byte, 1) == -1 || null_byte != 0)
        {
...
int _VocReadGetSound(VOCH handle, blocks *block)
{
    ...
    if (handle->voice_info.remaining_length > (blklens)MAX_SOUND_LENGTH)
        ...
        chunk_length = handle->voice_info.remaining_length;
    ...
    if (Bread(handle->file_handle, (char *)&block->dat.voice_data.data, (int)chunk_length) == -1)
    {
