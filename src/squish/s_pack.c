...
static word near InvokeFeatures(HAREA ha, HMSG hmsg, XMSG *pMsg, char *pCtrl, char *pszMsgBuf,
                                dword *pulMsgNum, int *dokill, int *logkill, int *rewriteit)
{
...
static unsigned near Pack_Netmail_Msg(HAREA sq, dword *mn, struct _cfgarea *ar)
{
    ...
    bytes = MsgReadMsg(mh, &msg, 0L, (dword)(maxmsglen - 1), msgbuf, (dword)ctlen, ctrl);
    ...
    else
    {
        ...
        if (DestIsHereA(&destnop) && (i = isave = GetCtrlToken(ctrl, "INTL")) != NULL)
        {
            NETADDR norig, ndest;
...
#ifdef OS_2
        if (InvokeFeatures(sq, mh, &msg, ctrl, msgbuf, mn, &dokill, &logkill, &rewriteit))
#endif
        {
    ...
    if (rewriteit)
    {
        ...
        if (AddrMatch(&msg.dest, &original.dest) && AddrMatch(&msg.orig, &original.orig))
        {
        ...
        else
        {
            ...
            if ((out_msg = MsgOpenMsg(sq, MOPEN_CREATE, 0L)) != NULL)
            {
                ...
                else
                {
                    ...
                    {
                        got = (unsigned)MsgReadMsg(mh, NULL, offset, (dword)REMAP_BUF_SIZE, buf, 0L,
                                                   NULL);

                        if (got == 0 || got == (unsigned)-1)
                            break;

                        (void)MsgWriteMsg(out_msg, TRUE, NULL, buf, (dword)got, MsgGetTextLen(mh),
                                          0L, NULL);
        ...
        for (sb = gr->nodes; sb; sb = sb->next)
            if (AddrMatchNS(&msg->dest, sb))
            {
                ...
                *olddest = msg->dest;
                ...
                (void)sprintf(intl, "\x01INTL %hu:%hu/%hu %hu:%hu/%hu\r", olddest->zone,
                              olddest->net, olddest->node, msg->orig.zone, msg->orig.net,
                              msg->orig.node);
...
static int near Send_Message(HMSG mh, XMSG *msg, dword bytes, dword mn, struct _cfgarea *ar)
{
    ...
    if (msg->attr & (MSGFILE | MSGFRQ | MSGURQ))
    {
        ...
        {
            S_LogMsg(":%s busy - %s:%ld will be packed later", Address(&msg->dest), ar->name, mn);
...
void HandleAttReqPoll(word action, byte **toscan)
{
    ...
    (void)printf("%s %s: ",
                 action == ACTION_GET      ? "Request file from"
...
#define TFL_DEL 2
    ...
    *pwd = 0;
    ...
    else
    {
        ...
        {
            if (filename[1] != ':' &&
                !strchr(filename, '\\')) /* wes - not changing; \\ is in packet (?) */
            {
                                      ...
                                      char *pwd)
{
    ...
    {
        if ((config.flag2 & FLAG2_QUIET) == 0)
            (void)printf(" (%sREQ %s)", (msg->attr & MSGFRQ) ? "F'" : "UPD", filename);
