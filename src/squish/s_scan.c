    ...
    *area = '\0';
    ...
    if (nmsg_scanned)
    {
        (void)printf("\nScanned %lu messages (%lu.%lu/second) and "
                     ...
                     nmsg_scanned, (unsigned long)nmsg_scanned / secs,
                     (unsigned long)(nmsg_scanned * 10Lu / secs) % 10Lu, (unsigned long)nmsg_sent,
...

{
    ...
    if (NetmailArea(ar) || BadmsgsArea(ar) || DupesArea(ar) || (ar->num_scan == 0 && !resc))
        return;
    ...
    while (next_sb)
    {
        ...
        while (next_sb && *next_sb)
        {
            if (*next_sb < ' ')
            {
                ...
                if (next_sb)
                    while (*next_sb == '\x0d' || *next_sb == '\x0a' || *next_sb == (byte)0x8d)
                        next_sb++;
            }
            else if (*next_sb == '\x0d' || *next_sb == '\x0a' || *next_sb == (byte)0x8d ||
                     *next_sb == ' ')
            {
                while (*next_sb == '\x0d' || *next_sb == '\x0a' || *next_sb == (byte)0x8d ||
                       *next_sb == ' ')
...
static void near PerformMessageUpdate(HAREA sq, struct _cfgarea *ar, XMSG *msg, char *ctrl,
                                      dword msgn, int do_modify, dword *hwm, unsigned added,
                    ...
                    dword *hwm, unsigned added, int *pfSkipScan)
{
    ...
    *do_kill = FALSE;
    ...
    {
        S_LogMsg("!Invalid ^aACUPDATE line in area %s (uid %lu)", ar->name, uid);
        return TRUE;
    }
        ...
        (msgn = FindUpdateMessage(sq, ar, msgid_hash, msgid_serial, &pNewHash, &pNewSerial)) != 0L)
    {
        ...
        PerformMessageUpdate(sq, ar, msg, ctrl_dup, msgn, do_modify, hwm, added, pNewHash,
                             pNewSerial);
    }
...
unsigned Scan_To_All(struct _cfgarea *ar, struct _sblist *sb, unsigned num_sb, XMSG *msg,
                     byte *ctxt, byte **afterkludge, UMSGID uid, int *do_kill, HAREA sq, dword *hwm)
{
    struct _groute *zg;
    ...
    if (!fSkipScan && (needscan || resc))
    {
        ...
        msg->attr &= (long)~(MSGSENT | MSGFWD | MSGORPHAN | MSGLOCAL | MSGXX2 | MSGREAD);

        scanmask = 0;

#ifdef DJ
        (void)fprintf(dj, "F:%0.15s T:%0.15s S:%20.20s D:%04d%02d%02d%02d%02d%02d\nS:", msg->from,
                      msg->to, msg->subj, msg->date_written.date.yr + 1980,
                      msg->date_written.date.mo, msg->date_written.date.da,
                      msg->date_written.time.hh, msg->date_written.time.mm,
             ...
             struct _statlist *sl)
{
    ...
    if (*msg->__ftsc_date)
        ...
        (void)sprintf(p, "%02d %s %02d  %02d:%02d:%02d",
...
static void near Add_Tear_Line(char *mbuf, struct _cfgarea *ar, XMSG *msg)
{
    ...
    if (config.pointnet && ar->primary.net == config.pointnet && config.addr->next)
    {
        addrstr = Address(SblistToNetaddr(config.addr->next, &n));
    }
    else
        addrstr = Address(SblistToNetaddr(&ar->primary, &n));
                            ...
                            struct _cfgarea *ar)
{
    ...
    if ((char huge *)((char huge *)cur_ob +
                      (unsigned)(sizeof(struct _pkthdr) + sizeof(struct _pktprefix) + lumplen +
                                 textlen)) >= (char huge *)(end_ob - 10))
    {
        Flush_Outbuf();
    }
...
void Flush_Outbuf(void)
{
    ...
    for (bl = (BLIST *)outbuf; bl->len; bl = bl->next)
    {
        ...
        if (!bl->done) /* If we haven't processed this one already */
        {
            ...
            (void)sprintf(addr, " (%u:%u/%u.%u)", bl->zone, bl->net, bl->node, bl->point);

            if ((config.flag2 & FLAG2_QUIET) == 0)
                (void)printf(addr);

#ifdef DIRECT_OB
            (void)sprintf(pktname, "%s%04hx%04hx.", FixOutboundName(config, bl->zone),
                          (unsigned)bl->net, (unsigned)bl->node);
            ...
            {
                (void)sprintf(pktname + strlen(pktname), "%cut", bl->flavour ? bl->flavour : 'O');
            }
            ...
            else /* Otherwise, create a new one */
            {
                                ...
                                writebufmax)) == NULL)
                {
                    ...
                    {
                        S_LogMsg("!Can't create packet %s (%d), freemem=%ld", pktname, errno,
                                 coreleft());
            ...
            if ((i = Bwrite(bf, &i, sizeof(word))) != sizeof(word) || Bclose(bf) != 0)
            {
                if (i != sizeof(word))
                    Bclose(bf);
