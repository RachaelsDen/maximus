...
void Format(void)
{
    ...
    printf("  -o        Convert Opus 1.1 (and some Opus 1.7) USER.DAT to Max " VERSION
           " format.\n");
...
void Dump(char *find)
{
    ...
    if ((huff = UserFileFindOpen(huf, *find ? find : NULL, *find ? find : NULL)) != NULL)
    {
        do
        {
            printf("%-21.21s %-25.25s %-6u %d\n", huff->usr.name, huff->usr.city, huff->usr.priv,
...
void Reverse_Max300(void)
{
    ...
    if ((huff = UserFileFindOpen(huf, NULL, NULL)) != NULL)
    {
        do
        {
            ...
            if (!UserFileUpdate(huf, usrold.name, usrold.alias, &huff->usr))
