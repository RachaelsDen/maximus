...
int Parse_Protocol(FILE *ctlfile, char *name)
{
    ...
    if (*pro.desc && do_prm)
    {
        ...
        Make_String(prm.protoname[protocol_num], name) prm.protocols[protocol_num] = 0;
        ...
        if (strings[prm.protocol_max])
            ...
            sprintf(temp, protocol_max, strings + prm.sys_path);

        if ((fd = open(temp,
                       O_CREAT | O_WRONLY | O_APPEND | O_BINARY | (protocol_num == 1 ? O_TRUNC : 0),
                       S_IREAD | S_IWRITE)) == -1)
        {
            printf("\n\aCan't open %s!\n", temp);
            exit(1);
