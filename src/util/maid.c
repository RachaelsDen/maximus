...
{
    printf("\rError %s %d: ********* %s%c '%s'\n", iname[n_include], linenum[n_include], msg,
           (args) ? ':' : 0, (args) ? args : "");
    if (fatal)
        exit(1);
    err = TRUE;
...
static void Process_Define(char *line)
{
    ...
    if ((defn[ndef].name = strdup(temp)) == NULL || (defn[ndef++].xlt = strdup(s)) == NULL)
    {
...
static void Compile_Language(HPRM hp)
{
    ...
    if (mfile)
        fprintf(mfile,
                "// %s: dynamic language definition file, generated by MAID v%s\n\n"
