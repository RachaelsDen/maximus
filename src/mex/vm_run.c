...
VMADDR EXPENTRY MexPtrToVM(void *ptr) { return (VMADDR)((char *)ptr - (char *)pbDs); }
...
void *EXPENTRY MexDSEG(VMADDR ofs) { return DSEG(ofs); }
...
static int near add_intrinsic_functions(unsigned short uscIntrinsic, struct _usrfunc *puf)
{
...
static int near proc_instruction(INST *inst)
{
    ...
    if (inst->opcode != QOP_FUNCRET)
    {
            ...
            inst->opcode != QOP_DWORD2BYTE && inst->opcode != QOP_DWORD2WORD)
        {
            ...
            {
                if (inst->arg2.addr.indirect && inst->opcode != QOP_ARG_VAL &&
                    inst->opcode != QOP_ARG_REF)
                {
                    got = inst->arg2.addr;
                    got.indirect = FALSE;
                ...
                case FormByte:
                    if (inst->flag & FLAG_ARG2_LIT)
                        ...
                        arg.b2 = *(byte *)fetch(FormByte, &inst->arg2.addr);
                    break;
                ...
                case FormWord:
                    if (inst->flag & FLAG_ARG2_LIT)
                        ...
                        arg.w2 = *(word *)fetch(FormWord, &inst->arg2.addr);
                    break;
                ...
                case FormDword:
                    if (inst->flag & FLAG_ARG2_LIT)
                        ...
                        arg.dw2 = *(dword *)fetch(FormDword, &inst->arg2.addr);
                    break;
                ...
                case FormString:
                    if (inst->flag & FLAG_ARG2_LIT)
                        ...
                        arg.a2 = *(IADDR *)fetch(FormString, &inst->arg2.addr);
                }
        }
    }
