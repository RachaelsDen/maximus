...
int BackPatch(PATCH *pat, VMADDR to_where)
{
    if (vm_output)
        ...
        return BackPatchTXT(pat, to_where);
}
...
static char *near ObjName(DATAOBJ *o, char *out)
{
    ...
    *out = '\0';
    ...
    if (o->objform == ObjformValue)
    {
        ...
        {
        case ByteKind:
            sprintf(out, "(%sbyte)'%c'", &"u"[o->type->fSigned],
                    (unsigned int)o -> form.val.kind.byteval);
            break;

        case WordKind:
            if (o->type->fSigned)
                sprintf(out, "(word)%d", (int)(sword)o->form.val.kind.wordval);
            else
                sprintf(out, "(uword)%u", (unsigned int)o->form.val.kind.wordval);
            break;

        case DwordKind:
            if (o->type->fSigned)
                sprintf(out, "(dword)%ld", (long)(sdword)o->form.val.kind.dwordval);
            else
                sprintf(out, "(udword)%lu", (unsigned long)o->form.val.kind.dwordval);
            break;
    ...
    if (o->name && !show_addr)
    {
        ...
        if (o->type)
            ...
            {
            case FormByte:
                sprintf(out, "(%sbyte)", &"u"[o->type->fSigned]);
                break;

            case FormWord:
                sprintf(out, "(%sword)", &"u"[o->type->fSigned]);
                break;

            case FormDword:
                sprintf(out, "(%sdword)", &"u"[o->type->fSigned]);
                break;
