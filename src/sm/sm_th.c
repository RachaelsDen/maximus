...
int CALLBACK RcIoMonitor(SMDISPATCH *psd)
{
    ...
    if ((newstream = malloc(psd->cbGot - sizeof(USHORT))) != NULL)
    {
        ...
        DbgPrintfp("Got video i/o for node %d (%d bytes)", *psd->pb, psd->cbGot - sizeof(USHORT));

        WinPostMsg(hwndClient, SM_VIO, MPFROM2SHORT(*psd->pb, psd->cbGot - sizeof(USHORT)),
                   MPFROMP(newstream));
    }
...
int CALLBACK RcIoMonitorDump(SMDISPATCH *psd)
{
    ...
    if ((newdump = malloc(usSize = usHeight * usWidth * sizeof(USHORT) + 5)) != NULL)
    {
        ...
        DbgPrintfp("Got video dump for node %d (%dx%d)", *psd->pb, usWidth, usHeight);

        WinPostMsg(hwndClient, SM_VIODUMP, MPFROMSHORT(*psd->pb), MPFROMP(newdump));
    }
...
void ThreadMon(char far *szPipe)
{
    ...
    DISPATCH dispatch[] = {{RCMSG_ACK, RcAck},
