...
static void OS2FAR *MAPIENTRY _palloc(size_t size) { return ((void OS2FAR *)malloc(size)); }

static void MAPIENTRY _pfree(void OS2FAR *ptr) { free(ptr); }
...
static void far *MAPIENTRY _farpalloc(size_t size) { return ((void far *)farmalloc(size)); }

static void MAPIENTRY _farpfree(void far *ptr) { farfree(ptr); }
...
static word near _CopyToBuf(byte *p, byte *out, byte **end, unsigned remaining)
{
    ...
    while (remaining > 0 && *p == '\x01' || strncmp(p, area_colon, 5) == 0)
    {
        ...
        while (remaining > 0 && *p && *p != '\x0d' && *p != '\x0a' && *p != (byte)0x8d)
        {
            if (out)
                *out++ = *p;
...
void MAPIENTRY MsgFreeCtrlBuf(char *cbuf) { pfree(cbuf); }
...
int far pascal farread(int handle, byte far *buf, unsigned len) { return read(handle, buf, len); }

int far pascal farwrite(int handle, byte far *buf, unsigned len) { return write(handle, buf, len); }
