...
int main(int argc, char *argv[])
{
    ...
    char ctxt[] = "\x01"
                  "MSGID: 1:249/106 12345678\x01"
                  "REPLY: 1:249/106.1 87654321\x01";
    char lastfrom[36] = "";
    ...
    if ((sq = MsgOpenArea(*name == '!' ? name + 1 : name, MSGAREA_NORMAL,
                          *name == '!' ? MSGTYPE_SDM : MSGTYPE_SQUISH)) == NULL)
    {
        ...
        if (*temp == 'Y' || *temp == 'y')
        {
            if ((sq = MsgOpenArea(*name == '!' ? name + 1 : name, MSGAREA_CRIFNEC,
                                  *name == '!' ? MSGTYPE_SDM : MSGTYPE_SQUISH)) == NULL)
            {
                printf("\aError opening SquishFile!\n");
                exit(ERL_ERROR);
    ...
    for (;;)
    {
        ...
        ReadText:
            ...
            {
                printf(
                    "From: %-20s (%u:%d/%d.%u)  Date: %s Private: %s\n"
                    "  To: %-20s (%u:%d/%d.%u)\n"
                    "Subj: %s\n"
                    "This: %ld (uid=%ld idx, %ld hdr), Down: %ld (uid=%ld), Up: %ld (uid=%ld)\n\n",
                    rmsg.from, rmsg.orig.zone, rmsg.orig.net, rmsg.orig.node, rmsg.orig.point,
                    sc_time((union stamp_combo *)&rmsg.date_written, temp),
                    (rmsg.attr & MSGPRIVATE) ? "Yes" : "No", rmsg.to, rmsg.dest.zone, rmsg.dest.net,
                    rmsg.dest.node, rmsg.dest.point, rmsg.subj, MsgCurMsg(sq),
                    MsgMsgnToUid(sq, MsgCurMsg(sq)), msgh->uidUs,
                    MsgUidToMsgn(sq, rmsg.replyto, UID_EXACT), rmsg.replyto,
                    MsgUidToMsgn(sq, rmsg.replies[0], UID_EXACT), rmsg.replies[0]);
        ...
        case 'l':
            while ((msgh = MsgOpenMsg(sq, MOPEN_READ, MSGNUM_next)) != NULL)
            {
                if (MsgReadMsg(msgh, &msg, 0L, 0L, NULL, 0L, NULL) == -1)
                    ...
                    printf("#%-3ld Fm: %-15.15s  To: %-15.15s  Subj: %-25.25s\r\n", MsgCurMsg(sq),
