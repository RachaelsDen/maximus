...
class BTREE : private BLKIOBUF
{
    ...
    int _remove_shift_keys(BTNODE *pbnNode, BTNODE *pbnSteal, BTNODE *pbnParent, int steal_idx,
                           int delete_right);
    int _remove_merge_3(BTNODE *pbnNode, BTNODE *pbnParent, BTNODE *pbnSteal, PNODE *pnNodeParents,
    ...
    unsigned _move_key(BTNODE *pbnTo, BTNODE *pbnFrom, BTNODE *pbnToParent, int parent_idx,
                       int delete_right);
    int destroy(NNUM nn);
  ...
  public:
    ...
    int CPPEXPORT open(char *pszFile, keycomp_t kf_ins, keycomp_t kf_srch, unsigned uiKeySz,
                       unsigned new_file, unsigned uiOrd = 8, unsigned fBuffer = TRUE);
    int CPPEXPORT close();
