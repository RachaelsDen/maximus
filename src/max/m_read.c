...
static int near ShowMessageHeader(XMSG *msg, word *msgoffset, long mn)
{
    ...
    Printf("%s%s%s %0.*s\n\n" CYAN, msg_subj_col,
           (msg->attr & (MSGFILE | MSGFRQ | MSGURQ)) ? files_colon : subj_colon, msg_subj_txt_col,
           TermWidth() - 10, Strip_Ansi(msg->subj, NULL, 0L));
...
int Msg_Display(HMSG msgh, XMSG *msg, int pause, char *areano, long msgnum, int offset, char *ctrl,
                int inbrowse)
{
    ...
    while ((got = Msg_Read_Lines(msgh, n_ol, wid, 0, ol, lt, &last_attr, MRL_QEXP)) > 0)
    {
        if ((kret = ShowMessageLines(got, ol, lt, &paged, msgoffset, pause, &nonstop, inbrowse)) <
            0)
        {
            Dealloc_Outline(ol);
    ...
    if (!nonstop && pause && (usr.bits2 & BITS2_MORE) &&
        display_line >
            (byte)(TermLength() - (menu_lines + 5 + ((usr.bits & BITS_FSR) ? -2 /*2u*/ : 0u))))
    {
...
static int near Msg_Get_Msgs(int dir, dword startmsg, int nonstop, int exact, int show_err)
{
    ...
    if (startmsg > MsgHighMsg(sq) && dir == DIRECTION_PREVIOUS && startmsg != 0xffffffffLu)
        startmsg = MsgHighMsg(sq);

    while (startmsg > 0 && startmsg <= MsgHighMsg(sq) && startmsg != 0xffffffffLu)
    {
        if ((msgh = MsgOpenMsg(sq, MOPEN_RW, startmsg)) != NULL)
        {
            if (MsgReadMsg(msgh, &msg, 0L, 0L, NULL, 0L, NULL) != -1 && CanSeeMsg(&msg))
            {
                ...
                ret = Msg_Display(msgh, &msg, nonstop ? FALSE : (usr.bits2 & BITS2_MORE), usr.msg,
                                  last_msg, 0, ctrl, FALSE);
    ...
    Printf(endavailmsg, (mah.ma.attribs & MA_PVT) == 0 ? blank_str : remain_pvt);
    Clear_KBuffer();
