...
#define SWFLAG_NOSCAN                                                                              \
    0x01 /* Don't scan lr ptr when entering area *                                                 \
          * (Switch_Msg_Area())                  */
...
#define VA_EXTONLY                                                                                 \
    0x0008 /* Used only in conj with VA_PWD.  If   *                                               \
...
#define MAX_AREAS                                                                                  \
    1296              /* Maximum number of total areas -- This #  *                                \
...
#define MAX_MSGDISPLAY                                                                             \
    26 /* Max # of lines that Msg_Display() will   *                                               \
        * req. from Msg_Read_Lines at once.        */
#define MAX_MSGLINES                                                                               \
    256                  /* Max # of lines you can req. from         *                             \
...
#define CHANGE_SCREENLEN                                                                           \
    85 /* # of #'s to print when testing user's    *                                               \
        * screen length.                           */
#define RETIMEOUT                                                                                  \
    60 /* #of seconds until hangup after 1st       *                                               \
        * timeout warning (1 min.)                 */
#define MAX_EXECARGS                                                                               \
    64                             /* Max. number of parameters when invoking  *                   \
                                    * an external program                      */
#define BMSGBUFLEN (MSGBUFLEN * 3) /* Amount to allow for msgbuffer            */
#define MSGBUFLEN                                                                                  \
    512                /* Max. num of characters of message to     *                               \
...
#define PREREGISTERED                                                                              \
    0xfffe /* Priv. value that means let no new        *                                           \
...
#define INPUT_ALREADYCH                                                                            \
    0x0010                           /* `ch' contains a character that has     *                   \
...
#define INPUT_NOCTRLC                                                                              \
    0x0040 /* Don't allow user to ^C to dump output  *                                             \
            * and redisplay prompt.                  */
#define INPUT_MSGENTER                                                                             \
    0x0080                    /* If we're currently entering message    *                          \
...
#define CINPUT_FULLPROMPT                                                                          \
    0x0200                       /* The GetListAnswer() o_prompt parm does  *                      \
                                  * include the choices                     */
...
#define LOCAL_EDIT                                                                                 \
    0x03 /* Returned ONLY by Editor() func, passed  *                                              \
          * back through Local_Editor().            */

#define HARD_CR                                                                                    \
    '\x0d' /* User pressed C/R here, insert into      *                                            \
            * the actual message.                     */
#define SOFT_CR                                                                                    \
    ((byte)'\x8d') /* Local wordwrap, don't bother storing    *                                    \
                    * it in the messages.                     */

#define MODE_UPDATE 0   /* Update position after wordwrap          */
#define MODE_NOUPDATE 1 /* Don't update position after wordwrap    */
#define MODE_SCROLL                                                                                \
    2 /* Update position, and scroll if          *                                                 \
       * necessary!                              */
...
#define MAX_LINELEN                                                                                \
    (LINELEN * 2)                     /* Double the length of line which can      *                \
