...
void GenerateActrack(XMSG *pxmsg, char *kend)
{
    ...
    sprintf(kend,
            "\x01"
            "ACTRACK: %02u%02u%02u%02u%02u%02u%02x%02x",
            (pm->date.yr + 80) % 100, pm->date.mo, pm->date.da, pm->time.hh, pm->time.mm,
            pm->time.ss, counter++ % 0x100, task_num);
}
                        ...
                        char *comment)
{
    ...
    if ((txt = malloc((size_t)txt_len)) != NULL)
    {
        if ((ctrl = malloc((size_t)ctrl_len)) != NULL)
        {
            ...
            MsgWriteMsg(hm, FALSE, pxmsg, txt, strlen(txt) + 1, txt_len, strlen(ctrl) + 1, ctrl);
