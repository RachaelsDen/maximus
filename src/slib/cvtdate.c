...
union stamp_combo *_fast TmDate_to_DosDate(struct tm *tmdate, union stamp_combo *dosdateUA)
{
    union stamp_combo *dosdate = alignStatic(dosdateUA);
...
char *_fast sc_time(union stamp_combo *scUA, char *string)
{
    ...
    {
        sprintf(string, "%2d %s %02d %02d:%02d:%02d", sc->msg_st.date.da,
                months_ab[sc->msg_st.date.mo - 1], (80 + sc->msg_st.date.yr) % 100,
