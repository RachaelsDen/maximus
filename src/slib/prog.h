...
#if !defined(PROJECT_BIG_ENDIAN) && !defined(PROJECT_LITTLE_ENDIAN) && !defined(UNIX)
/* Do OS/2, DOS or Windows run on non-Intel, non-Alpha CPUs?? */
#define PROJECT_LITTLE_ENDIAN
#endif

#if defined(PROJECT_BIG_ENDIAN) && defined(PROJECT_LITTLE_ENDIAN)
#error PROJECT_BIG_ENDIAN and PROJECT_LITTLE_ENDIAN cannot both be defined at the same time!
#endif

#if !defined(PROJECT_BIG_ENDIAN) && !defined(PROJECT_LITTLE_ENDIAN)
#error Either PROJECT_BIG_ENDIAN or PROJECT_LITTLE_ENDIAN must be defined!
#endif
...
#define MAX_DRIVES                                                                                 \
    26 /* Maximum number of drives on system;     *                                                \
...
#define SH_DENYNONE SH_DENYNO
    ...
    !defined(__IBMC__) && !defined(__TOPAZ__) && !defined(_lint)
