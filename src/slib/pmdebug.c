...
void DbgBegin(HWND hwndOwner, HAB hab, USHORT usMaxItem, USHORT fMinimize)
{
    ...
    static ULONG flFrameFlags = FCF_TITLEBAR | FCF_SIZEBORDER | FCF_MINMAX | FCF_SYSMENU;
    static BOOL fInit = 0;
    USHORT cx, cy;
    ...
    if (!fInit)
    {
        WinRegisterClass(hab, szDebugClass, (PFNWP)DebugProc, (ULONG)CS_SIZEREDRAW, 0);
        fInit = TRUE;
    }

    cItem = 0;
    cMaxItem = usMaxItem;
    hwndDbgFrame = WinCreateStdWindow(hwndOwner, 0, &flFrameFlags, szDebugClass, "Debug Window", 0L,
                                      (HMODULE)0, 0, &hwndDbgClient);
    ...
    WinSetWindowPos(hwndDbgFrame, (HWND)0, 10, /*swp.x+swp.cx/10,*/
                    10,                        /*swp.y+swp.cy/10,*/
                    cx / 2, cy / 4,
...
static MRESULT EXPENTRY DebugProc(HWND hwnd, MSGDEF msg, MPARAM mp1, MPARAM mp2)
{
    ...
    case WM_SIZE:
        ...
        WinSetWindowPos(hwndDbgList, (HWND)0, 0, 0, SHORT1FROMMP(mp2), SHORT2FROMMP(mp2),
                        SWP_SIZE | SWP_MOVE);
        break;
...
void DbgPrint(char *txt)
{
    ...
    WinSendMsg(hwndDbgList, LM_SELECTITEM, MPFROMSHORT(cItem - 1), MPFROMSHORT(TRUE));
}
...
void DbgPrintp(char *txt)
{
    ...
    WinPostMsg(hwndDbgList, LM_INSERTITEM, MPFROMSHORT(LIT_END), MPFROMP(strdup(txt)));
