...
char *cdecl dstrdup(char *s)
{
    ...
    if (table)
    {
        if (!shutup)
            fprintf(efile,
                    "STRD SZ=" PTR_SIZE " FR=" PTR_TEXT " MM=" PTR_TEXT " SP=" SPC_TEXT " \"%s\"\n",
                    (unsigned long)size, POINTER(MAGICSTACK(s)), POINTER(p), (long)coreleft(), s);
...
void cdecl dfree(void *b)
{
    ...
    if (table)
    {
        ...
        if (!shutup)
            ...
            fprintf(efile, "DFRE ------- FR=" PTR_TEXT " MM=" PTR_TEXT " SP=" SPC_TEXT "\n",
#endif
                    POINTER(MAGICSTACK(b)), POINTER(block), (long)coreleft());
...
void *cdecl drealloc(void *b, size_t size)
{
    ...
    if (table)
    {
        if (!shutup)
            fprintf(efile,
                    "DRLC SZ=" PTR_SIZE " FR=" PTR_TEXT " MM=" PTR_TEXT " SP=" SPC_TEXT
                    " OL=" PTR_TEXT "\n",
                    (unsigned long)size, POINTER(MAGICSTACK(b)), POINTER(p), (long)coreleft(),
                    POINTER(b));
