...
word _fast VidOpen(int has_snow, int desqview, int dec_rows)
{
    ...
    if (dec_rows)
    {
        ...
        {
            VidScroll(SCROLL_up, 1, Vid_Attribute, 0, 0, (char)(Vid_NumCols - 1),
                      (char)Vid_NumRows);
...
void _fast VidBios(int use_bios) { Vid_Bios = (char)use_bios; }
...
void pascal VidSyncDVWithForce(int fForce) { VidSyncDV(); }
