...
void _fast noise(int freq, int duration) { DosBeep(freq, duration); }
...
void _fast noise(int freq, int duration)
{
    ...
    if (freq)
    {
        ...
        outportb(TIMER_PORT, inportb(TIMER_PORT) & 0xfc);
...
void _fast noise(int freq, int duration) { Beep((DWORD)freq, (DWORD)duration); }
