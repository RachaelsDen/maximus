...
static int populateFF(FFIND *ff, const char *filename, struct stat *sb_p)
{
    ...
    TmDate_to_DosDate(localtime_r(&sb.st_ctime, &timebuf),
                      &ff->scCdate); /* C = creation? last change? */
...
FFIND *FindOpen(char *filespec, unsigned short attribute)
{
    ...
    ff->globFlags =
        GLOB_NOSORT | GLOB_NOESCAPE | GLOB_PERIOD; /* Get as close to DOS wildcarding as possible */
...
void main(int argc, char **argv) { walk(PATH_DELIMS); /* start at root*/ }
