...
long _stdc ntell(int fd) { return (nlseek(fd, 0, SEEK_CUR)); }
...
int _stdc nsopen(const char *name, int mode, int shacc, ...)
{
    ...
    r.h.al = (char)((mode & O_RDWR) == O_RDWR ? 0x02 : (mode & O_WRONLY) == O_WRONLY ? 0x01 : 0x00);
...
long _stdc nlseek(int fd, long ofs, int pos)
{
    ...
    r.h.al = (char)((pos == SEEK_CUR) ? 1 : (pos == SEEK_END) ? 2 : 0);
#ifdef __FLAT__
    r.x.ebx = fd;
    r.w.cx = (int)(ofs >> 16);
...
int _stdc nsopen(const char *name, int mode, int shacc, ...)
{
    ...
    rc = DosOpen(p, &hf, &usAction, 0,
                 /****/
                 (permiss & S_IWRITE) ? FILE_NORMAL : FILE_READONLY,
                 /****/
                 ((mode & O_CREAT) ? OPEN_ACTION_CREATE_IF_NEW : OPEN_ACTION_FAIL_IF_NEW) |
    ...
    rc = DosOpen(p, &hf, &usAction, 0,
                 ...
                 ((mode & O_EXCL)
                      ? 0
                      : ((mode & (O_CREAT | O_TRUNC)) == (O_CREAT | O_TRUNC) ? 0 : FILE_OPEN)) |
