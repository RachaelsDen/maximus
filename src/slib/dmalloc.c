...
int shutup =
    0; /* hack by wes to satisfy linking in maid -- normally defined in pmdmal.c for os/2?? */
...
static void add_table(void *p, unsigned long ulWhere, size_t size)
{
    ...
    for (i = 0; i < cPtrs; i++)
    {
        if (ptab[i].p == p)
        {
            dmalloc_err("add_table - pointer %08lx already in table (from %08lx)", p,
                        ptab[i].ulFrom);
        }
    }
...
static void remove_table(void *p)
{
    ...
    for (i = 0; i < cPtrs; i++)
    {
        if (p == ptab[i].p)
        {
            ...
            {
                dmalloc_err("remove_table - key grunged for pointer %08lx - from %08lx", p,
                            ptab[i].ulFrom);
            }
...
void check_table()
{
    ...
    for (i = 0; i < cPtrs; i++)
    {
        ...
        if (*(unsigned long *)pszP != FIRST_KEY ||
            *(unsigned long *)(pszP + sizeof(unsigned long) + ptab[i].size) != LAST_KEY)
        {
            dmalloc_err("check_heap - key grunged for pointer %08lx - from %08lx", pszP,
                        ptab[i].ulFrom);
        }
    }
}
