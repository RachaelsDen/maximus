...
DeclareCommand(SE, 240, "End of subnegotiation parameters") DeclareCommand(
    ...
    DeclareCommand(IP, 244, "Interrupt") DeclareCommand(AO, 245, "Abort Output") DeclareCommand(
        AYT, 246, "Are you there?") DeclareCommand(EC, 247, "Erase Character")
        DeclareCommand(EL, 248, "Erase Line") DeclareCommand(GA, 249, "Go Ahead")
            DeclareCommand(SB, 250, "Subnegotiation Follows") DeclareCommand(WILL, 251, "Will ")
                DeclareCommand(WONT, 252, "Will not") DeclareCommand(DO, 253, "Please do")
                    DeclareCommand(DONT, 254,
                                   "Please do not") DeclareCommand(IAC, 255, "Interpret as Command")
#endif

#ifdef DeclareOption
                        DeclareOption(TRANSMIT_BINARY, 0, 0, "Transmit Binary")
                            DeclareOption(ECHO, 1, 1, "echo") DeclareOption(SGA, 3, 2,
                                                                            "suppress go ahead")
                                DeclareOption(STATUS, 5, 3, "status") DeclareOption(TIMING, 6, 4,
                                                                                    "timing mark")
