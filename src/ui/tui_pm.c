...
void TuiPmInit(void)
{
    ...
    WinSetWindowPos(hwndFrame, (HWND)0, 0, 0, 0, 0, SWP_MAXIMIZE);

    WinSetWindowPos(hwndFrame, (HWND)0, (SHORT)-cx, /* x position */
...
#define DLT_SIZE 14
    ...
    *txt++ = '\0';
    pdi->flStyle = /*WS_VISIBLE |*/ WS_CLIPSIBLINGS | WS_SAVEBITS | FS_NOBYTEALIGN | FS_DLGBORDER;
    ...
    *(ULONG *)txt = 0;
    ...
    for (i = 0; i < (SHORT)hd->num_opt; i++, pdi++)
    {
        ...
        {
            pdi->offClassName = 3;
            pdi->flStyle = WS_VISIBLE | WS_GROUP | WS_TABSTOP | BS_PUSHBUTTON;
        }
        else if (hd->opt[i].regist == DlgRadReg)
        {
            pdi->offClassName = 3;
            pdi->flStyle = WS_VISIBLE | WS_GROUP | WS_TABSTOP | BS_AUTORADIOBUTTON;
        }
        else if (hd->opt[i].regist == DlgChkReg)
        {
            pdi->offClassName = 3;
            pdi->flStyle = WS_VISIBLE | WS_GROUP | WS_TABSTOP | BS_AUTOCHECKBOX;
        }
...
static MRESULT EXPENTRY DlgProc(HWND hwnd, MSGDEF msg, MPARAM mp1, MPARAM mp2)
{
    ...
    case WM_INITDLG:
        ...
        WinSetWindowPos(hwnd, (HWND)0, (p1.x - (rcl.xRight - rcl.xLeft)) / 2,
                        (p1.y - (rcl.yTop - rcl.yBottom)) / 2, 0, 0, SWP_MOVE);
