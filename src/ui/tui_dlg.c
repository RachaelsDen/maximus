...
MenuFunction(DlgStrShow)
{
    ...
    WinPutstra(win, opt->cy, opt->cx, col(opt->parent->col_item), str);

    if (sel_ch)
        WinPutch(win, opt->cy, opt->cx + sel_ofs, sel_ch, col(opt->parent->col_hot));
    ...
    else
    {
        ...
        if (strlen(opt->data) > field_len /*&& !high*/)
            WinPutch(win, opt->cy, opt->cx + strlen(str) + field_len - 1, '\xaf',
                     col(high ? STR_SEL : STR_LEFTRIGHT)); /* ae */
    }
...
static void near DlgRedrawStr(HVOPT opt, word *offset)
{
    ...
    */
    ...
    WinPrintf(win, "%-*.*s", field_len - 2, field_len - 2, str + start_pos);
...
MenuFunction(DlgRadReg)
{
    ...
    _TuiMenuAddKey(opt, VKEY_DOWN, NULL, next, NULL, 0, 0);
    _TuiMenuAddKey(opt, K_ENTER,
                   NULL /*DlgRadAct*/ /* it's not SAA, but this is less confusing for DOS users */,
                   last + 1, NULL, 0, 0);
...
MenuFunction(DlgChkReg)
{
    ...
    _TuiMenuAddKey(opt, K_ENTER,
                   /*DlgChkAct*/ NULL /* it's not saa, but it's less confusing for DOS users */,
...
static void near DlgIntWrap1(HVOPT opt)
{
    if (!opt->appdatap)
    {
        ...
        if (opt->appflag & AF_BYTE)
            ...
            sprintf(szNumTemp, "%u", *(word *)opt->appdatap);
    }
}
...
static void near DlgIntWrap2(HVOPT opt)
{
    if (opt->appdatap)
    {
        if (opt->appflag & AF_BYTE)
            ...
            *(word *)opt->appdatap = atoi(opt->data);
...
MenuFunction(DlgValAct)
{
    ...
    h->start_y =
        opt->cy + opt->parent->start_y + (opt->parent->border != BORDER_NONE) - 1 - h->def_opt;
