...
sword _TuiDrawOption(HVOPT opt, char col_norm, char col_hot, word draw_hot)
{
    ...
    optname = _TuiMakeOptName(opt, &selch, &selcol,
                              (opt->parent->type & MENU_VERT) ? opt->parent->wid : 0);
...
sword _TuiAddEnterFunc(HVMENU vmenu, HVOPT opt)
{
    ...
    ret = _TuiMenuAddKey(
        opt, K_ENTER, opt->menufn,
        ((vmenu->type & MENU_DIALOG) && opt->regist != DlgButReg && opt->regist != DlgLstReg)
...
static struct _axlt
{
    ...
    {'A', K_ALTA}, {'B', K_ALTB}, {'C', K_ALTC}, {'D', K_ALTD}, {'E', K_ALTE}, {'F', K_ALTF},
    {'G', K_ALTG}, {'H', K_ALTH}, {'I', K_ALTI}, {'J', K_ALTJ}, {'K', K_ALTK}, {'L', K_ALTL},
    {'M', K_ALTM}, {'N', K_ALTN}, {'O', K_ALTO}, {'P', K_ALTP}, {'Q', K_ALTQ}, {'R', K_ALTR},
    {'S', K_ALTS}, {'T', K_ALTT}, {'U', K_ALTU}, {'V', K_ALTV}, {'W', K_ALTW}, {'X', K_ALTX},
    {'Y', K_ALTY}, {'Z', K_ALTZ}, {'1', K_ALT1}, {'2', K_ALT2}, {'3', K_ALT3}, {'4', K_ALT4},
    {'5', K_ALT5}, {'6', K_ALT6}, {'7', K_ALT7}, {'8', K_ALT8}, {'9', K_ALT9}, {'0', K_ALT0},
    {0, 0},
};
    ...
    for (opt = menu->opt, end = opt + menu->num_opt; opt < end; opt++)
    {
        ...
        *dummyopt = *opt;
        ...
        if (opt->regist != DlgInfReg)
        {
            ...
            {
                _TuiMenuAddKey(dummyopt, sel_char, opt->menufn, opt, opt->menu, HOT_NONE,
                               VKF_AFTER);
            }
            ...
            if (sel_char && (menu->type & MENU_HOT))
            {
                ...
                if ((mkey = AsciiToScanCode(sel_char)) != 0)
                {
                    ...
                    {
                        _TuiMenuAddKey(dummyopt, 0xffff, NULL, opt, opt->menu, HOT_PRESS1,
                                       VKF_AFTER);

                        _TuiMenuAddKey(dummyopt, mkey, opt->menufn, opt, opt->menu, HOT_RELEASE1,
                                       VKF_AFTER);
                    }
                    else
                    {
                        _TuiMenuAddKey(dummyopt, mkey, NULL /*opt->menufn*/, opt, opt->menu,
            ...
            if (!did_hot)
            {
                ...
                if (!did_hot)
                    _TuiMenuAddKey(dummyopt, 0xffffu, NULL, opt, NULL, HOT_PRESS1, VKF_AFTER);
            }
        }
...
sword _TuiRegisterSubordinate(HVMENU oldmenu, HVOPT opt)
{
    ...
    if (opt->menu->type & MENU_PLACE)
    {
        ...
        {
            opt->menu->start_x = oldmenu->start_x + opt->cx + (oldmenu->border != BORDER_NONE);

            opt->menu->start_y = oldmenu->start_y + 1 + (oldmenu->border == BORDER_NONE ? 0 : 1);
        }
        else
        {
...
HVMENU _TuiRegisterMenu1(HVMENU vmenu, HVKEY keylist)
{
    ...
    for (opt = vmenu->opt, last_opt = NULL, end_opt = vmenu->opt + vmenu->num_opt; opt < end_opt;
         opt++)
    {
...
static sword near _TuiExecOption(HVMENU menu, HVKEY key, HVMENU *nxtmen)
{
    ...
    if (key->menufn)
    {
        ...
        ret = (*key->menufn)((key->flag & VKF_AFTER) == 0 || !key->newopt ? menu->curopt
                                                                          : key->newopt);
...
static sword near _TuiGetOption(HVMENU menu, HVMENU *nextmen)
{
    ...
    while ((key = _TuiGetKeyboardOption(menu)) == NULL && (key = _TuiGetMouseOption(menu)) == NULL)
#ifdef OS_2
        DosSleep(1L);
#else
        ;
...
sword _fast TuiExecMenu(HVMENU vmenu)
{
    ...
    do
    {
        ...
        if (!_TuiMenuOnStack(nextmen, menustk))
        {
            ...
            if ((nextmen->win = WinOpen(nextmen->start_y, nextmen->start_x, rows, cols,
                                        nextmen->border, col(nextmen->col_win),
                                        col(nextmen->col_bor), WIN_NOCSYNC | WIN_NODRAW)) == NULL)
            {
                ret = -1;
                break;
            }
