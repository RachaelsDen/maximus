...
static sword MAPIENTRY SdmCloseArea(HAREA mh)
{
    ...
    if (Mhd->hwm_chgd)
        if ((msgh = SdmOpenMsg(mh, MOPEN_CREATE, 1L)) != NULL)
        {
            ...
            (void)SdmWriteMsg(msgh, FALSE, &msg, msgbody, strlen(msgbody) + 1, strlen(msgbody) + 1,
                              0L, NULL);
...
static dword MAPIENTRY SdmReadMsg(HMSG msgh, PXMSG msg, dword offset, dword bytes, byte *text,
                                  dword clen, byte *ctxt)
{
    ...
    if (msg)
    {
        ...
        if (farread(msgh->fd, (char *)&fmsg, sizeof(struct _omsg)) != (int)sizeof(struct _omsg))
        {
            msgapierr = MERR_BADF;
            return (dword)-1L;
        }
...
static sword MAPIENTRY SdmKillMsg(HAREA mh, dword msgnum)
{
    ...
    for (mn = 0; mn < (unsigned)mh->num_msg; mn++)
        if (Mhd->msgnum[mn] == (unsigned)msgnum)
        {
            (void)memmove(Mhd->msgnum + mn, Mhd->msgnum + mn + 1,
...
static dword MAPIENTRY SdmUidToMsgn(HAREA mh, UMSGID umsgid, word type)
{
    ...
    for (mn = 0; mn < (unsigned)mh->num_msg; mn++)
        if (Mhd->msgnum[mn] == wmsgid || (type == UID_NEXT && Mhd->msgnum[mn] >= wmsgid) ||
...
static dword MAPIENTRY SdmGetTextLen(HMSG msgh)
{
    ...
    if (msgh->msg_len == (dword)-1)
    {
        ...
        msgh->msg_len =
            (end < sizeof(struct _omsg)) ? 0L : (unsigned long)(end - (dword)sizeof(struct _omsg));
...
static sword near _Grab_Clen(HMSG msgh)
{
    return ((sdword)SdmReadMsg(msgh, NULL, 0L, 0L, NULL, 0L, NULL) < (sdword)0 ? -1 : 0);
}
...
static sword near _SdmRescanArea(HAREA mh)
{
    ...
    if ((ff = FindOpen(temp, 0)) != 0)
    {
        ...
        do
        {
            ...
            {
                Mhd->msgnum =
                    repalloc(Mhd->msgnum, (Mhd->msgnum_len += SDM_BLOCK) * sizeof(unsigned));
...
static void MSGAPI Convert_Xmsg_To_Fmsg(PXMSG msg, struct _omsg *fmsg)
{
    ...
    if (*msg->__ftsc_date)
    {
        ...
        (void)sprintf(fmsg->date, "%02d %s %02d  %02d:%02d:%02d",
