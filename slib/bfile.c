...
static int near Brefill(BFILE b)
{
    ...
    iGot = nread(b->fd, b->pcBufEnd, b->stBufSize - (size_t)(b->pcBufEnd - b->pcBuf));
...
long _fast Bseek(BFILE b, long lRelPos, int fWhere)
{
    ...
    case BSEEK_SET:
        ...
        break;
    case BSEEK_CUR:
        lNewPos = b->lPos + (b->pcBufCur - b->pcBuf) + lRelPos;
        break;
    case BSEEK_END:
...
long _fast Btell(BFILE b) { return b->lPos + (b->pcBufCur - b->pcBuf); }
