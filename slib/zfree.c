...
long _stdc zfree(char *path)
{
    ...
    GetDiskFreeSpace(szDrive, &lSectorsPerCluster, &lBytesPerSector, &lFreeClusters, &lClusters);
...
static long CalcFreeSpace(long lSectorsPerCluster, long lBytesPerSector, long lFreeClusters)
{
    ...
    if (lBytesPerCluster * lFreeClusters > rc || ((lBytesPerCluster * lFreeClusters) & 0xfffff000))
    {
        rc = INT_MAX;
    }
