...
static void _intr trap_break(void) { brk_trapped++; /* inc every time user hits ^break */ }
                                      ...
                                      PCONTEXTRECORD pcr, PVOID pv)
{
    ...
    case XCPT_INVALID_UNWIND_TARGET:
        ...
        outfn("type: %08lx address: %08lx time: %08lx\n", (long)perepr->ExceptionNum,
              (long)perepr->ExceptionAddress, (long)time(&ulTime));
        ...
        if (pcr->ContextFlags & CONTEXT_SEGMENTS)
        {
            ...
            outfn("ds=%04x  es=%04x  fs=%04x  gs=%04x", pcr->ctx_SegDs, pcr->ctx_SegEs,
        ...
        if (pcr->ContextFlags & CONTEXT_INTEGER)
        {
            ...
            outfn("esi=%08lx  edi=%08lx", pcr->ctx_RegEsi, pcr->ctx_RegEdi);

            if (pcr->ContextFlags & CONTEXT_CONTROL)
                outfn(" ebp=%08lx  efl=%08lx", pcr->ctx_RegEbp, pcr->ctx_EFlags);
...
static VOID PASCAL FAR BreakHandler(USHORT usSigArg, USHORT usSigNum)
{
    ...
    {
        DosSetSigHandler(BreakHandler, &pfnb, &actb, SIGA_ACCEPT, SIG_CTRLBREAK);
    }
    else
    {
        DosSetSigHandler(BreakHandler, &pfnc, &actc, SIGA_ACCEPT, SIG_CTRLC);
    }
...
void _fast brktrap(void) { int fix_me_later; }

void _stdc brkuntrap(void) { int write_me_later; }
