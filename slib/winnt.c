...
void pascal _WinBlitzNT(VWIN *win, int row, /* starting row (screen) */
...
void _fast WinSync(VWIN *win, int sync_cursor)
{
    ...
    for (; x < max; x++)
    {
        ...
        if (flag || win->dleft[x] != -1)
        {
            ...
            if (!Vid_Bios &&
                /*x >= 0 &&*/ x < (sword)win->s_height + (win->border != BORDER_NONE) &&
                win->blitz[x])
            {
                int col, num_cols;
...
VWIN *_fast WinOpen(int row, int col, int height, int width, int border, int attr, int battr,
                    int flag)
{
    ...
    if ((win->rowtable = win->orig_rt = (byte far **)malloc(height * sizeof(char far *))) == NULL)
    {
        free(win);
        return NULL;
    }
    ...
    if (flag & WIN_INHERIT) /* Inherit underlying text (transparent) */
    {
        ...
        ReadConsoleOutput(hStdout, (PCHAR_INFO)win->buf, cDestSize, cDestCoord, &sr);
...
void _fast WinScrl(VWIN *win, int dir, int srow, int scol, int erow, int ecol, int attr)
{
    ...
    if (dir == SCROLL_down)
    {
        for (x = erow; x > srow; x--)
        {
            fmemmove((char far *)WinOfs(win, x, scol), (char far *)WinOfs(win, x - 1, scol),
...
int main()
{
    ...
    for (i = 0; i < 100; i++)
    {
        ...
        sprintf(temp,
                "This is test %d of window code.  I wonder if it will scroll the screen okay?\r\n",
                i);
        WinPuts(wscrn, temp);
        WinSync(wscrn, TRUE);
    }
