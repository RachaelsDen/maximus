...
static char rcs_id[] = "$Id: mcp_cli.c,v 1.1.1.1 2002/10/01 17:53:27 sdudley Exp $";
...
int CALLBACK ClKey(CINFO *pci)
{
    ...
    McpSendMsg(si[i].ppi->hp, RPMSG_KEY, pci->pbBuf + sizeof(USHORT) + 1, pci->cbMsg - 3);
...
void ClientIO(HPIPE hp)
{
    ...
    DISPATCH dt[] = {{CLMSG_BEGINMONITOR, ClBeginMonitor},
