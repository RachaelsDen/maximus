...
static char rcs_id[] = "$Id: mcp.c,v 1.1.1.1 2002/10/01 17:53:26 sdudley Exp $";
...
void far PipeMain(void far *arg)
{
    ...
    static DISPATCH dt[] = {{PMSG_SET_STATUS, MSetStatus},
...
#ifdef __FLAT__
    rc = DosCreateNPipe(szPipeName, &ppi->hp, NP_ACCESS_DUPLEX | NP_NOINHERIT | NP_WRITEBEHIND,
                        NP_WAIT | NP_UNLIMITED_INSTANCES | NP_TYPE_MESSAGE | NP_READMODE_MESSAGE,
                        4096, /* output buffer */
                        8192, /* input buffer */
                        250);
#else
    rc = DosMakeNmPipe(szPipeName, &ppi->hp, NP_ACCESS_DUPLEX | NP_NOINHERIT | NP_WRITEBEHIND,
                       NP_WAIT | NP_UNLIMITED_INSTANCES | NP_TYPE_MESSAGE | NP_READMODE_MESSAGE,
                       4096, /* output buffer */
                       8192, /* input buffer */
                       250);
    ...
    if (rc)
    {
        MCPLog("!%02d PipeMain: SYS%04d: DosMakeNmPipe(\"%s\")", ppi->session, rc, szPipeName);
...
#endif
            MCPLog("!%02d PipeMain: SYS%04d: DosConnectNmPipe", ppi->session, rc);
...
static void near StartThread(void(far *f)(void far *arg), int session, void *arg)
{
    PVOID pv;
    USHORT rc;
    ...
    {
        MCPLog("!%02d StartThread: SYS%04d: DosAllocMem(%u)", session, rc, THREAD_STK_SIZE);
        DosExit(EXIT_PROCESS, 1);
    }
#else
    SEL sel;
    ...
    {
        MCPLog("!%02d StartThread: SYS%04d: DosAllocSeg(%u)", session, rc, THREAD_STK_SIZE);
        DosExit(EXIT_PROCESS, 1);
    }
...
void far ClientMain(void far *arg)
{
...
#ifdef __FLAT__
    rc = DosCreateNPipe(szClientPipeName, &hp, NP_ACCESS_DUPLEX | NP_NOINHERIT | NP_WRITEBEHIND,
                        NP_WAIT | NP_UNLIMITED_INSTANCES | NP_TYPE_MESSAGE | NP_READMODE_MESSAGE,
                        4096, /* output buffer */
                        8192, /* input buffer */
                        250);
...
#else
    rc = DosMakeNmPipe(szClientPipeName, &hp, NP_ACCESS_DUPLEX | NP_NOINHERIT | NP_WRITEBEHIND,
                       NP_WAIT | NP_UNLIMITED_INSTANCES | NP_TYPE_MESSAGE | NP_READMODE_MESSAGE,
                       4096, /* output buffer */
                       8192, /* input buffer */
                       250);
    ...
    if (rc)
    {
        MCPLog("!CL SYS%04d: DosMakeNmPipe(\"%s\")", rc, szPipeName);
...
#endif
        MCPLog("!CL SYS%04d: DosConnectNmPipe", rc);
...
static void Synchronize(void)
{
    ...
    rc = DosCreateMutexSem("\\sem32\\maximus\\mcp", &hmtx, DC_SEM_SHARED, FALSE);
