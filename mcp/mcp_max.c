...
static char rcs_id[] = "$Id: mcp_max.c,v 1.1.1.1 2002/10/01 17:53:27 sdudley Exp $";
...
int CALLBACK MSetStatus(PINFO *ppi)
{
    ...
    MCPLog("=%02d Max Node %d; User %s; Status %s", ppi->session, si[ppi->session].tid,
           si[ppi->session].cs.username, si[ppi->session].cs.status);
...
int CALLBACK MMaxSendMsg(PINFO *ppi)
{
    ...
    MCPLog("#%02d Max Dispatch msg %d -> %d", ppi->session, pcd->tid, pcd->dest_tid);
    ...
    if (i == max_ses)
    {
        MCPLog("!%02d Max Msg from node %d destined to unknown node %d!", ppi->session, pcd->tid,
               pcd->dest_tid);
        return 0;
    }
    ...
    MCPLog("#%02d Max Sent msg %d -> %d, len=%d", ppi->session, pcd->tid, pcd->dest_tid, usGot);
...
int CALLBACK MQueryTaskInfo(PINFO *ppi)
{
    ...
    if (i == max_ses)
        ...
        memmove(&cs, &si[i].cs, sizeof cs);

    MCPLog(":%02d Max Query status for node %d (user %s)", ppi->session, ppi->pbBuf[2],
           cs.username);
    ...
    if (rc)
        MCPLog("!%02d Max SYS%04d: DosWrite MMQueryActive", ppi->session, rc);
...
int CALLBACK MVio(PINFO *ppi)
{
    ...
    for (i = 0; i < MAX_MCP_MONITORS; i++)
        if (si[ppi->session].ahpMonitors[i])
            McpSendMsg(si[ppi->session].ahpMonitors[i], RCMSG_IO_MONITOR, ppi->pbBuf + 1,
                       ppi->cbMsg - 1);
...
int CALLBACK MVioDump(PINFO *ppi)
{
    ...
    for (i = 0; i < MAX_MCP_MONITORS; i++)
        if (si[ppi->session].ahpMonitors[i])
        {
            McpSendMsg(si[ppi->session].ahpMonitors[i], RCMSG_IO_MONITOR_DUMP, ppi->pbBuf + 1,
                       ppi->cbMsg - 1);
        }
...
int CALLBACK MLog(PINFO *ppi)
{
  ...
  MCPLog("!%02d Got log line \"%s\"", ppi->session, ppi->pbBuf+2);
    ...
    for (i = 0; i < MAX_MCP_MONITORS; i++)
        if (si[ppi->session].ahpMonitors[i])
            McpSendMsg(si[ppi->session].ahpMonitors[i], RCMSG_LOG, ppi->pbBuf + 2, ppi->cbMsg - 2);
