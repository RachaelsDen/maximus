...
static void near QWKGetMsgid(HAREA sq, UMSGID replyto, char *reply_kludge)
{
    ...
    *reply_kludge = 0;
    ...
    if ((ctrl_len = MsgGetCtrlLen(hmsg)) > 0 && (ctrl_buf = malloc(ctrl_len)) != NULL)
    {
...
static int near QWKTossMsgBody(PXMSG msg, struct _qmhdr *qh, int msg_blocks, int qfd, char *block,
                               char **pkludge, int *pfUpdateStatus)
{
#ifdef MAX_TRACKER
