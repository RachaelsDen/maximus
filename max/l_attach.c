...
int dwordcmp(void OS2FAR *a1, void OS2FAR *a2) { return recncmp(a1, a2); }
...
#define atFILENM                                                                                   \
    (sizeof(dword) + LFA_ASCII_NAME_SIZE + MAX_ALEN + sizeof(UMSGID) + LFA_ASCII_NAME_SIZE)
#define atATTRIB                                                                                   \
    (sizeof(dword) + LFA_ASCII_NAME_SIZE + MAX_ALEN + sizeof(UMSGID) + LFA_ASCII_NAME_SIZE +       \
     LFA_ASCII_FILE_SIZE)
#define atDATEAT                                                                                   \
    (sizeof(dword) + LFA_ASCII_NAME_SIZE + MAX_ALEN + sizeof(UMSGID) + LFA_ASCII_NAME_SIZE +       \
     LFA_ASCII_FILE_SIZE + sizeof(dword))
#define atDATERC                                                                                   \
    (sizeof(dword) + LFA_ASCII_NAME_SIZE + MAX_ALEN + sizeof(UMSGID) + LFA_ASCII_NAME_SIZE +       \
     LFA_ASCII_FILE_SIZE + sizeof(dword) + sizeof(SCOMBO))
