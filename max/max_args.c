    ...
    if (*arg == '-' || *arg == '/')
    {
        ...
        case 'm': /* Multitasker */
        {
            static struct _mtask
            {
                char letter;
                char mtask;
            } mtasks[] = {{'d', MULTITASKER_doubledos}, {'u', MULTITASKER_unix},
                          {'q', MULTITASKER_desqview},  {'p', MULTITASKER_topview},
                          {'l', MULTITASKER_mlink},     {'w', MULTITASKER_mswindows},
                          {'m', MULTITASKER_pcmos},     {'a', MULTITASKER_auto},
                          {'n', MULTITASKER_none},      {0, 0}};
...
static void near ReadCallerInfo(char *path)
{
    ...
    if (read(fd, (char *)&crec, sizeof(crec)) == sizeof(crec) && crec.iversion == CREC_VER)
    {
        waitforcaller = FALSE;
