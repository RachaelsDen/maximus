...
static sword near DisplayFilesBbs(DSTK *d)
{
    ...
    for (;;)
    {
        ...
        if (d->automore && display_col == 1 && DispMoreYnBreak(d->nonstop, NULL, d->type))
        {
            d->ret = DRET_BREAK;
            break;
        }
...
word near DCQuest(DSTK *d) { return (DisplayQuestionnaire(d) == SKIP_FILE); }
...
word DispSlowGetChar(DSTK *d) { return (DispGetChar()); }
