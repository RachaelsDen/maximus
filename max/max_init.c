...
void Init_Variables(void)
{
    ...
    *firstname = *linebuf = *searchfor = *fix_menupath = *last_readln = *arq_info = '\0';
    ...
    snoop = keyboard = caller_online = do_timecheck = fossil_initd = written_echomail =
        written_matrix = sent_time_5left = sent_time_almostup = wrap = inmagnet = restart_system =
            first_search = barricade_ok = create_userbbs = this_logon_bad = inchat = locked =
                chatreq = do_useredit = fthru_yuhu = shut_up = debug_ovl = no_dcd_check = FALSE;
    fLoggedOn = FALSE;
...
char *Startup(void)
{
    ...
    if (~version[0] + ~version[2] + ~version[3] != VER_CHECKSUM ||
        ...
        ~tear_ver[0] + ~tear_ver[2] + ~tear_ver[3] != VER_CHECKSUM ||
        ~us_short[0] + ~toupper(us_short[1]) + ~toupper(us_short[2]) != NAME_CHEKSUM ||
    ...
    case VIDEO_BIOS:
        ...
        {
#endif
            dspwin = WinMsg(BORDER_DOUBLE, col.pop_text, col.pop_border, logo1, logo2, NULL);
    ...
    if (*PRM(menupath))
        ...
        strcpy(menupath, PRM(sys_path));

    if (*PRM(rippath))
        strcpy(rippath, PRM(rippath));
    else
        strcpy(rippath, menupath);
...
void Read_Prm(char *ctlname)
{
    ...
    if (read(prmfile, (char *)&prm, sizeof(prm)) != sizeof(prm) || prm.id != 'M' ||
...
static void near StartUpVideo(void)
{
    ...
    if ((win = WinOpen(0, 0, VidNumRows() - ((prm.flags & FLAG_statusline) ? 1 : 0), VidNumCols(),
                       BORDER_NONE, CGRAY, CGRAY, 0)) == NULL)
    {
        logit(mem_none);
        Local_Beep(3);
