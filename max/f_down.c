...
#define NAMEBUFLEN 255
    ...
    do
    {
        ...
        {
            Inputf(namebuf,
                   (((do_tag == TAG_ONELINE && usr.video) ? INPUT_NOLF : 0) | INPUT_LB_LINE), 0,
                   NAMEBUFLEN - 1, file_enter_tag, FileEntries() + 1);
...
static int near GotFile(char *fname, long size, int do_tag, sword protocol, int flags)
{
    ...
    Printf(file_stats, thisfile + 1, No_Path(fname), (int)(this_time / 60L), (int)(this_time % 60L),
           (long)size);
...
word File_Send_Files_Sub(sword protocol, char *mname, char *newuppath, long realbytes, int flag)
{
    ...
    {
        if (!ZmInitStatics() || (cr3tab = mkcrc32tab()) == NULL || (crctab = mkcrc16tab()) == NULL)
        {
            if (crctab)
                (free)(crctab);
...
#define SPEC_COND 0x4242u
                    ...
                    ch = ZmodemSendFile(temp, FALSE, (long)iFilesLeft, lBytesLeft);
                               ...
                               long *prtime)
{
    ...
    *prtime = XferTime(protocol, *realbytes);
    ...
    if (!acsflag(CFLAGA_NOTIME))
    {
        ...
        if ((dword)(time(NULL) + pvtime) > timeoff || (dword)(time(NULL) + *prtime) > getoff)
        {
