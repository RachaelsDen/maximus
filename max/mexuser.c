...
{
    if (pmisThis->huf != NULL || (pmisThis->huf = UserFileOpen(PRM(user_file), 0)) != NULL)
        return TRUE;
    return FALSE;
}
...
word EXPENTRY intrin_userfindopen(void)
{
    ...
    if (OpenUser())
    {
        ...
        if ((pmisThis->huff = UserFileFindOpen(pmisThis->huf, (szName && *szName) ? szName : NULL,
                                               (szAlias && *szAlias) ? szAlias : NULL)) != NULL)
        {
...
word EXPENTRY intrin_userfindnext(void) { return UserFind(TRUE); }
word EXPENTRY intrin_userfindprev(void) { return UserFind(FALSE); }
...
word EXPENTRY intrin_userupdate(void)
{
    ...
    if (OpenUser())
    {
        ...
        else if (UserFileUpdate(pmisThis->huf, (szName && *szName) ? szName : NULL,
                                (szAlias && *szAlias) ? szAlias : NULL, &user))
            regs_2[0] = TRUE;
    }
...
word EXPENTRY intrin_userremove(void)
{
    ...
    if (OpenUser())
    {
        ...
        MexImportUser(u, &user);
        if (strcmp(user.name, usr.name) != 0 && UserFileRemove(pmisThis->huf, &user))
            regs_2[0] = TRUE;
    }
