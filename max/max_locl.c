...
static int near Priv_Adjust_Start(void)
{
    ...
    if ((privwin = WinOpen(0, 0, 11, 35 + wid, BORDER_DOUBLE, col.pop_text, col.pop_border,
    ...
    if ((vp = WinCreatePickList(privwin, 1, 1, 5, col.pop_list, col.pop_lselect, pl_privs,
                                usr.priv)) == NULL)
    {
        WinClose(privwin);
        privwin = NULL;
...
static int near Parse_Local_Normal(int ch)
{
    ...
    case '=':
#ifdef CANENCRYPT
        LocalMsg(pwd_is, (usr.bits & BITS_ENCRYPT) ? brackets_encrypted : cfancy_str(usr.pwd));
#else
    ...
    */
...
#endif
    {
        ...
        if ((dspwin = WinOpen(((VidNumRows() - 19) >> 1) - 1, 1, 19, 78, BORDER_DOUBLE,
                              col.pop_text, col.pop_border, WIN_NOCSYNC | WIN_NODRAW)) == NULL)
        {
            free(temp);
            return;
        }
...
void Priv_Up(void)
{
    ...
    if (++ucls < classes && (ClassGetInfo(ucls, CIT_ACCESSFLAGS) & CFLAGA_HANGUP) == 0)
    {
