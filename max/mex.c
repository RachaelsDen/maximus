...
static void near MexStoreMarea(struct mex_marea *pma, PMAH pMah)
{
    ...
    StoreString(MexPtrToVM(pma), struct mex_marea, descript,
                pMah->heap ? PMAS(pMah, descript) : "");
    ...
    StoreString(MexPtrToVM(pma), struct mex_marea, attach_path,
                pMah->heap ? PMAS(pMah, attachpath) : "");
    MexKillStructString(mex_marea, pma, barricade);
    StoreString(MexPtrToVM(pma), struct mex_marea, barricade,
                pMah->heap ? PMAS(pMah, barricade) : "");
...
static void near MexStoreFarea(struct mex_farea *pfa, PFAH pFah)
{
    ...
    StoreString(MexPtrToVM(pfa), struct mex_farea, descript,
                pFah->heap ? PFAS(pFah, descript) : "");
    MexKillStructString(mex_farea, pfa, downpath);
    StoreString(MexPtrToVM(pfa), struct mex_farea, downpath,
                pFah->heap ? PFAS(pFah, downpath) : "");
    ...
    StoreString(MexPtrToVM(pfa), struct mex_farea, filesbbs,
                pFah->heap ? PFAS(pFah, filesbbs) : "");
    MexKillStructString(mex_farea, pfa, barricade);
    StoreString(MexPtrToVM(pfa), struct mex_farea, barricade,
                pFah->heap ? PFAS(pFah, barricade) : "");
...
#define StoreByteStringUser(field, val, size)                                                      \
    StoreByteString(MexPtrToVM(pusr), struct mex_usr, field, val, size)
...
int MexStartupIntrin(char *pszFile, char *pszArgs, dword fFlag)
{
    ...
    else
    {
        ...
        rc = MexExecute(pszFile, pszArgs, fFlag, N_INTRINFUNC, _intrinfunc, intrin_setup,
                        intrin_term, MexLog, intrin_hook_before, intrin_hook_after);
