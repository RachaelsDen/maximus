...
static struct _cgs *near ChatFindOpen(void)
{
...

    {
        ...
        rc = DosTransactNmPipe(hpMCP, (PBYTE)&usQuery, sizeof usQuery, tids, sizeof tids, &cbRead);
...
int ChatFindIndividual(byte tid, char *username, char *status, word *avail)
{
    ...
    *(USHORT *)req = PMSG_QUERY_TASKINFO;
    ...
    rc = DosTransactNmPipe(hpMCP, req, sizeof req, (PBYTE)&cstat, sizeof cstat, &cbRead);
...
void Who_Is_On(void)
{
    ...
    while (ChatFindNext(cgs, &tid, username, status, NULL))
    {
        ...
        Printf(hu_is_on_3, username, tid, status, eqstri(username, usrname) ? ch_you : blank_str);
        ...
        for (;;)
        {
            ...
            else /* Must have been a text string */
            {
                ...
                *(word *)prompt = cur_ch;
                ...
                for (x = 0; x < num_task; x++)
                    if (ChatSendMsg((byte)tasks[x], CMSG_CDATA, strlen(prompt + 2) + 3, prompt) ==
                        -1)
#ifdef DEBUG
