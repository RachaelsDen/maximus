    ...
    if ((fp = shfopen(badname, fopen_read, O_RDONLY)) != NULL)
    {
        while (ret == FALSE && fgets(badname, PATHLEN - 1, fp) != NULL)
        {
            ...
            if (*p && *p != ';')
            {
                ...
                for (p = strtok(badname, cmd_delim); ret == FALSE && p; p = strtok(NULL, cmd_delim))
                {
                    ret = MatchWC(p, name);
                }
            }
...
#else /* !BINK_PROTOCOLS */
    else if (!ZmInitStatics() || (cr3tab = mkcrc32tab()) == NULL || (crctab = mkcrc16tab()) == NULL)
    {
        if (crctab)
            (free)(crctab);
        ...
        if (carrier() && ul_start_time)
        {
            ...
            if (temp_long != 0)
            {
                ...
                if (lReward)
                {
                    ...
                    if (temp_long)
                        Printf(time_added_for_ul, (long)(temp_long / 60L), (long)(temp_long % 60L));
                }
            }
        }
...
static int near Check_Filename(char *ulpath, char *fname)
{
    ...
    if (!(acsflag(CFLAGA_ULBBSOK)) &&
            ((p = stristr(fname, dotbbs)) != NULL || (p = stristr(fname, dotgbs)) != NULL ||
             (p = stristr(fname, dotrbs)) != NULL) ||
        (p = stristr(fname, filesdat_name)) != NULL ||
        (p = stristr(fname, filesdmp_name)) != NULL || (p = stristr(fname, filesidx_name)) != NULL)
    {
