...
void Word_Left(void)
{
    ...
    if (!isalnumpunct(screen[offset + cx][cy]) || !isalnumpunct(screen[offset + cx][cy - 1]) ||
        cy == 1 || cy > strlen(screen[offset + cx] + 1))
    {
        while (((!isalnumpunct(screen[offset + cx][cy])) || cy > strlen(screen[offset + cx] + 1)) &&
               cy > 1)
            cy--;
...
void Word_Right(void)
{
        ...
        cy == 1 || cy > strlen(screen[offset + cx] + 1))
    {
        ...
        if (cy >= strlen(screen[offset + cx] + 1) && cy <= cursor_y)
            ...
            while (!isalnumpunct(screen[offset + cx][cy]) && screen[offset + cx][cy] &&
                   cy < usrwidth)
                cy++;
    }
    else
        while ((!isalnumpunct(screen[offset + cx][cy])) && cy < usrwidth)
            cy++;
