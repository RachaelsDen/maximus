...
static void near TrackModComment(TRK t, TRK_MSG_NDX *ptmn, dword msgnum, char *szComment)
{
    ...
    sprintf(szNewComment,
            "\x01"
            "ACCOMMENT: %s",
            temp);

    TrackInsertTracking(sq, msgnum, NULL, FALSE, FALSE, NULL, FALSE, NULL, TRUE,
                        *temp ? szNewComment : NULL);
}
...
static void near TrackModStatus(TRK t, TRK_MSG_NDX *ptmn, dword msgnum, HAREA ha)
{
    ...
    else
    {
        ...
        TrackMakeACAUDIT(comment, trk_status_set, TrkGetStatus(t, ptmn), usrname);

        TrackInsertTracking(ha, msgnum, NULL, FALSE, FALSE, NULL, TRUE, comment, FALSE, NULL);
    }
}
...
static void near TrackModPriority(TRK t, TRK_MSG_NDX *ptmn, dword msgnum)
{
    ...
    else
    {
        ...
        TrackMakeACAUDIT(comment, trk_pri_set, TrkGetPriority(t, ptmn), usrname);
            ...
            if (kludges)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        } while (ch != mod_keys[3] && ch != mod_keys[4] && ch != 13 && ch != 0);
                    }
                }
