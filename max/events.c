...
static void near Event_Syntax_Err(char *err) { logit(log_event_err, err); }
...
static void near Parse_Day(struct _diskevent *e, char *word)
{
    ...
    for (;;)
    {
        ...
        if (eqstri(text, "all"))
        {
            e->day |= DAY_SUN | DAY_MON | DAY_TUE | DAY_WED | DAY_THU | DAY_FRI | DAY_SAT;
        }
...
static int near TryReadEither(int e_num, int write_anyway)
{
    ...
    if (!fexist(ebbsn) || GTdate(&ebbs_st, &edat_st))
    {
        ...
        Read_Event_File(e_num); /* read binary format directly */
