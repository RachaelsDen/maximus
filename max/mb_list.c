...
int List_Display(BROWSE *b)
{
    ...
    */
    ...
    Printf(br_list_format, (prm.flags2 & FLAG2_UMSGID) ? MsgMsgnToUid(b->sq, b->msgn) : b->msgn,
           (MsgToThisUser(b->msg.to) && ((mah.ma.attribs & MA_NET) == 0 || MsgToUs(&b->msg.dest)) &&
            (b->msg.attr & MSGREAD) == 0)
               ? br_msg_new
               : ((b->msg.attr & MSGPRIVATE) ? br_msg_pvt : br_msg_notnew),
           MsgToThisUser(b->msg.from) ? LRED : YELLOW, Strip_Ansi(b->msg.from, NULL, 0),
           MsgToThisUser(b->msg.to) ? LRED : LGREEN, Strip_Ansi(b->msg.to, NULL, 0),
