...
void File_Raw(void)
{
    ...
    for (ff = FindOpen(filespec, ATTR_SUBDIR), ret = 0;
         ff && ret == 0 && (display_col != 1 || !MoreYnBreak(&nonstop, NULL)); ret = FindNext(ff))
    {
        stamp = ff->scWdate;

        sprintf(date, "%s %d, %d", months[stamp.msg_st.date.mo - 1], stamp.msg_st.date.da,
        ...
        Printf("%-12.12s %s     %-18s %02d:%02d\n", ff->szName, size, date, stamp.msg_st.time.hh,
