...
static int near Quote_Read(void)
{
    ...
    while (lines_displayed < QUOTELINES)
    {
        ...
        qbl = Msg_Read_Lines(qmh, QUOTELINES - lines_displayed, wid, swid, p,
                             linetype + lines_displayed, &last_msg_attr, MRL_QEXP);
        ...
        for (temp = lines_displayed; temp < QUOTELINES; temp++)
        {
            ...
            if (linetype[temp] & MSGLINE_SEENBY && !GEPriv(usr.priv, prm.seenby_priv))
                continue;
            else if (linetype[temp] & MSGLINE_KLUDGE && !GEPriv(usr.priv, prm.ctla_priv))
                continue;
            ...
            if (QuoteThisLine(s2))
                ...
                Printf(norm_format, len, msg_quote_col, s2);
            ...
            {
                while (lines_displayed < QUOTELINES)
                    Printf(norm_format, usrlen + 1 + lines_displayed++, msg_quote_col, blank_str);
