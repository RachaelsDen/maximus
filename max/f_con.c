...
void File_Contents(void)
{
    ...
    *filename = '\0';
    ...
    if (File_Okay(filename))
    {
        ...
        sprintf(filespec, ss,
                (IsInFilesBbs(&fah, filename, NULL, path) && *path) ? path : FAS(fah, downpath),
                filename);
...
static sword near Display_Contents(char *filename)
{
    ...
    if (*buffer == 'P' && *(buffer + 1) == 'K')
        ret = Read_Zip(arcfile);
             ...
             (lh0->method[2] == 'z' || lh0->method[2] == 'h') && isdigit(lh0->method[3]) &&
             lh0->method[4] == '-')
...
#endif
    {
        ...
        if (type == ARCHIVE_LZH)
        {
            ...
            else       /* version 2 */
            {
                ...
                if ((hsize = do_ext_hdr(-1, &fname, lhtemp + sizeof(struct _lh2), lh2->ext_size)) ==
                    -1)
                {
                    break;
                }
        ...
        {
            if (read(lzhfile, (char *)&archead, sizeof(struct _archead)) != sizeof(struct _archead))
            {
                break;
            }
        ...
        if (type != ARCHIVE_ARJ || !first)
        {
            ...
            Printf(lzh_type, p, uncompressed_size, compressed_size, percent,
                   (int)(1000 - ((compressed_size * 1000) / uncompressed_size)) % 10,
                   file_date.msg_st.date.mo, file_date.msg_st.date.da,
                   (file_date.msg_st.date.yr + 80) % 100, file_date.msg_st.time.hh,
                   file_date.msg_st.time.mm, file_date.msg_st.time.ss << 1, attr, method, crc);
        }
