                                   ...
                                   PMAH pmah)
{
    ...
    while (fbgetsc(temp, PATHLEN, upfile, &is_trimmed))
    {
        ...
        for (s = temp; *s; s++)
        {
            ...
            {
                if (s[1] && !isspace(s[1]) &&
                    ((((char *)s > (char *)temp) && s[-1] != ' ') ||
                     (((char *)s == (char *)temp) && !first && last_char != ' ')))
            ...
            if (temp[1] > 32 && !last_isblstr && last_len >= 60)
            {
                if (!is_wd((char)last_char)) /* Make sure wrapped line has space */
                    *temp = ' ';
                    ...
                    strocpy(temp, temp + 1); /* shift back one space */
...
static void near SaveMsgFromEditor(HMSG msgh, long total_len, PMAH pmah)
{
    ...
    for (line = 1; line <= num_lines; line++)
    {
        ...
        MsgWriteMsg(msgh, TRUE, NULL, screen[line] + 1, strlen(screen[line] + 1), total_len, 0L,
                    NULL);
    }
    ...
    if (num_lines && screen[num_lines] && screen[num_lines][0] == SOFT_CR)
    {
        ...
        MsgWriteMsg(msgh, TRUE, NULL, &c, (long)sizeof(char), total_len, 0L, NULL);
    }
                                  ...
                                  char *kludge, HAREA ha)
{
    ...
    if (pmah->ma.attribs & MA_NET)
        sprintf(temp, " (%d:%d/%d.%d)", msg->dest.zone, msg->dest.net, msg->dest.node,
                msg->dest.point);
    else
        *temp = '\0';

    logit(chg ? chgdmsg : msgto, msg->to, temp, msgarea, save_to);
...
#define ADDR_LEN 30
        ...
        (!first && *(byte *)line >= 32 && *(byte *)line < 127))
    {
        ...
        do
        {
            ...
            while (*p && (!isdigit(*p) || (isdigit(*p) && p[-1] != ' ')) && *p != ',' &&
                   *p != ';' && (*p != '.' || p[-1] != ' '))
            {
                *o++ = *p++;
            }
