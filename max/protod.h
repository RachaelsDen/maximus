...
void Scan_For_Intl_And_Fmpt(int msgfile, int *m_zone, int *m_net, int *m_node, int *m_point,
                            char *str_msgid, char *str_fmpt, int intl_num);
...
int Outside(char *leaving, char *returning, int method, char *parm, int display_slogan, int ctltype,
...
word File_Okay(char *filename);
sword ShowFileEntry(word *type, char *nstop, word isoffline, SCOMBO *pscFile, SCOMBO *pscUl,
                    byte *filename, long size, int ck_abort, byte *desc, byte *descwork, PFAH pfah);
...
sword GetNewDateDiscrete(union stamp_combo *new_date, union stamp_combo *last, char *promptprefix,
                         int *piOrigYear, int *piOrigMonth, int *piOrigDay);
