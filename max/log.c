...
int LogOpen(void)
{
    ...
    if (!logfile && log_name && log_name[0])
    {
        ...
        if ((logfile = sfopen(log_name, fopen_append,
                              O_APPEND | O_TEXT | O_WRONLY | O_CREAT | O_NOINHERIT, SH_DENYWR)) ==
            NULL)
        {
...
void cdecl logit(char *format, ...) { NW(format); }
...
void cdecl logit(char *format, ...)
{
    ...
    if ((string = malloc(LOGIT_SIZE + 20)) == NULL || (string2 = malloc(LOGIT_SIZE)) == NULL)
    {
        if (string)
            free(string);
    ...
    sprintf(string2, "%c %02d:%02d:%02d %s\n", *p, localt->tm_hour, localt->tm_min, localt->tm_sec,
            p + 1);
    ...
    sprintf(string2, logformat, *p, localt->tm_mday, months_ab[localt->tm_mon], localt->tm_hour,
            localt->tm_min, localt->tm_sec, nameabbr, p + 1);
    ...
    if (log_wfc && displaymode == VIDEO_IBM)
        WFC_LogMsg(string2);
    else if ((!snoop && !local) || !caller_online || in_file_xfer || screen_log || *string2 == '!')
    {
        /*    if (*string2 != '@' || debuglog)*/
        if (*string2 != '@')
        {
