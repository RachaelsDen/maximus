...
word DisplayMaxCode(DSTK *d)
{
    ...
    case 'u':
        vbuf_flush();
        DispGetToBlank(d, d->scratch);
        play_tune(PRM(tune_file), d->scratch, wait_for_it, multitasker == MULTITASKER_desqview);
        break;
...
static int near MsgAreaControl(DSTK *d)
{
    ...
    case 'E':
        if (usr.video != GRAPH_TTY && usr.width >= 79 && usr.len >= 24 &&
...
static word near DispCheckExpire(DSTK *d)
{
    ...
    if (usr.xp_flag & XFLAG_EXPDATE)
    {
        ...
        return (usr.xp_date.ldate && GEdate(&sc, &usr.xp_date) ? SKIP_NONE : SKIP_LINE);
    }
    else
    {
...
static word near DispHandleXtern(DSTK *d)
{
    ...
    Outside(NULL, NULL, ch | rr, p, FALSE, CTL_NONE, RESTART_DOTBBS, d->lastfile);
...
static word near DispCheckBaud(DSTK *d)
{
    ...
    if ((ch == '1' && baud < 1200L) || (ch == '2' && baud < 2400L) || (ch == '9' && baud < 9600L))
    {
        return SKIP_LINE;
    }
...
static word near DispNodeAvailable(DSTK *d)
{
    ...
    return ((word)(ChatFindIndividual(tid, NULL, NULL, &avail) && avail) ? SKIP_NONE : SKIP_LINE);
}
...
static word near DispComment(void) { return (!Goodbye_Comment() ? SKIP_LINE : SKIP_NONE); }
