...
static void near Max_Version(void)
{
    ...
    Printf(LRED "System: %s\n"
                " SysOp: %s\n\n",
           gkey_info + strlen(gkey_info) + 1, gkey_info);
#endif
...
int UserList(void)
{
    ...
    if (go)
    {
        ...
        do
        {
            ...
            if (!*search || (searchname && stristr(user.name, search)) ||
                (*user.alias && searchalias && stristr(user.alias, search)))
            {
                int hiddenuser =
                    (user.bits & BITS_NOULIST) ||
                    (ClassGetInfo(ClassLevelIndex(user.priv), CIT_ACCESSFLAGS) & CFLAGA_HIDDEN);
                ...
                if ((showallusers || !hiddenuser) && !(user.delflag & UFLAG_DEL))
                {
                    ...
                    Printf(ul_format,
                           ((prm.flags & FLAG_alias) && *user.alias) ? user.alias : user.name,
                           sc_time(&user.ludate, string), user.city);
                }
            }
