...
static int near NodeExist(NETADDR *d)
{
    ...
    for (offset = 0L; (y = read(idxfile, (char *)ndi, sizeof(*ndi) * NUM_NDI)) > sizeof(*ndi);
         offset += (long)y)
    {
...
int ReadNode(NETADDR *d, void *nodeptr)
{
    ...
    else
    {
        ...
        lseek(nlfile, ((long)offset) * (prm.nlver == 6 ? (long)n6size : (long)sizeof(struct _node)),
              SEEK_SET);

        if (read(nlfile, nodeptr,
                 prm.nlver == 6 ? sizeof(struct _newnode) : sizeof(struct _node)) <= 0)
        {
            close(nlfile);
            logit(cantread, temp);
            return FALSE;
