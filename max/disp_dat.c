...
word DisplayDatacode(DSTK *d)
{
    ...
    case 1: /* Quote */
        ...
        do
        {
            ...
            if (!p) /* end-of-file, so recycle to beginning */
                ...
                bstats.quote_pos = ftell(quotefile);
    ...
    case 17: /* Total number of system calls (ordinal) */
        Printf("%lu%s", bstats.num_callers, Ordinal(bstats.num_callers));
        break;
    ...
    case 20: /* Current Time */
        strftime(d->scratch, 15, "%H:%M:%S", localtime(((longtime = time(NULL)), &longtime)));
