...
static int near add_fileattach(char *szFileName, dword ulAttachID)
{
    ...
    for (p = strtok(szFileName, cmd_delim); rc && p; p = strtok(NULL, cmd_delim))
    {
    ...
    if (file_attaches)
    {
        ...
        for (q = qwk_atts; q; q = q->next)
        {
            ...
            if (lcopy(q->fname, temp) == -1)
            {
                ...
                AddFileEntry(temp, FFLAG_NOBYTES, -1L);
        }
    }
    return rc;
}
...
int QWK_Begin(BROWSE *b)
{
    ...
    if ((qidxs = malloc(sizeof(struct _qmndx) * MAX_QIDX)) == NULL ||
        ...
        Make_QWK_Directory() == -1 || Create_Control_DAT() == -1 || Create_Messages_DAT() == -1 ||
...
static int near Create_Control_DAT(void)
{
    ...
    fprintf(cdat, "%02d-%02d-%4d,%02d:%02d:%02d\n", sc.msg_st.date.mo, sc.msg_st.date.da,
            sc.msg_st.date.yr + 1980, sc.msg_st.time.hh, sc.msg_st.time.mm, sc.msg_st.time.ss << 1);
...
static int near AddPersonalIndex(BROWSE *b, struct _qmndx *pqn)
{
    ...
    if (eqstri(b->msg.to, usr.name) || (*usr.alias && eqstri(b->msg.to, usr.alias)))
    {
...
static int near QWKInitializeHeaders(BROWSE *b, int qwkfile, unsigned long *pthis_rec)
{
    int rc = 0;
                                  ...
                                  TRK_MSG_NDX *ptmn, char *ctrl, int *pdo_we_own)
{
    ...
    *pblpos = block;
    ...
    {
        sprintf(*pblpos, "From: %s" QWK_EOL_STR QWK_EOL_STR, Address(&b->msg.orig));
...
int QWK_After(BROWSE *b)
{
    ...
    else
    {
        ...
        if (!b->fSilent)
        {
            ...
            Printf(qwk_pack_fmt, MAS(mah, name), MAS(mah, descript), num_msg, area_toyou,
                   area_packed);
        }
...
static int near QWK_Compress_Mail(BROWSE *b)
{
    ...
    else
    {
        ...
        ret = Outside(NULL, NULL, OUTSIDE_RUN | OUTSIDE_NOFIX, cmd, FALSE, CTL_NONE, 0, NULL);
    }
...
static void near Flush_Len_Chain(void)
{
    ...
    {
        lseek(qwkfile, (lc->rec * QWK_RECSIZE) + offsetof(struct _qmhdr, len), SEEK_SET);
