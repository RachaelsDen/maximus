...
void Redraw_StatusLine(void)
{
    ...
    Printf(max_status, mmsg->to, mmsg->subj + (strnicmp(mmsg->subj, "Re:", 3) == 0 ? 4 : 0));
...
void Redraw_Quote(void)
{
    ...
    if (quoting)
    {
        ...
        for (temp = 0; temp < QUOTELINES; temp++)
            Printf(quote_format, usrlen + 1 + temp, msg_quote_col, initials,
                   quotebuf + (temp * MAX_LINELEN));
        ...
        if (!Mdm_keyp() || cx == cursor_x + offset)
        {
            ...
            if (cx != max_lines)
            {
                if (cx > max_lines || screen[cx] == NULL ||
                    (cx <= num_lines && screen[cx][cy] == '\0' && screen[cx][1]))
                {
                    /*          Putc(' '); */
                }
                else
    ...
    for (; col >= (word)(usrwidth - MAX_WRAPLEN); col--)
    {
        if (is_wd(scx[col]) || col == (word)(usrwidth - MAX_WRAPLEN))
        {
            ...
            if (screen[offset + cx][1])
            {
                ...
                strocpy(screen[offset + cx] + 1 + strlen(scx + col), screen[offset + cx] + 1);
                ...
                if ((mode == MODE_SCROLL /*|| mode==MODE_UPDATE*/) && (cursor_x + 1 >= usrlen))
                ...
                if (y == 0 || (col <= (word)strlen(screen[offset + cursor_x] + 1) + 1 &&
                               !is_wd(screen[offset + cursor_x][col]) && screen[offset + cx][1]))
                {
                    zerolen = TRUE;
                }
                else
            ...
            {
                memmove(screen[offset + cx] + 1, scx + col, y = strlen(scx + col) + 1);
