                                    ...
                                    FDAT *dp)
{
    ...
    ret = ShowFileEntry(attr, nonstop, (dp->flag & FF_OFFLINE), &dp->fdate, &dp->udate, fnam,
                        dp->fsize, TRUE, desc, descwork, pfah);
...
static sword near LocateShowFile(char *nstop, PFAH pfah, FDAT *dp, byte *curdmp, int dmp,
                                 dword *dmploc, word *dmpb, word *attr)
{
...
static sword near LocateReadFdat(FDAT *da, int dat, int dmp, word attr, char *nstop, PFAH pfah)
{
    ...
    while (ret == 0 && (got = read(dat, (char *)da, sizeof(FDAT) * MAX_SEARCH_DAT)) >= sizeof(FDAT))
    {
        got /= sizeof(FDAT);
        ...
        for (dp = da, dend = da + got; dp < dend; dp++)
        {
            ...
            if (doit)
            {
                ...
                ret = LocateShowFile(nstop, pfah, dp, curdmp, dmp, &dmploc, &dmpbytes, &attr);
            }
...
void File_Locate(void)
{
         ...
         colour++)
    {
        ...
        ret = LocateSearchArea(*searchfor == '*' ? DISPLAY_NEWFILES : DISPLAY_SEARCH, &nonstop, &fa,
                               colour);
