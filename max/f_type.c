...
void File_Type(void)
{
    ...
    if (*buf == '\x1a' || eqstrn(buf, "MZ", 2) || eqstrn(buf, "PK", 2) || eqstrn(buf, "GIF", 3) ||
        stristr(filespec, dot_zip) || stristr(filespec, dot_pak) || stristr(filespec, dot_lzh) ||
        stristr(filespec, dot_arc) || stristr(filespec, dot_arj))
    {
        bad = TRUE;
    }
    else
