...
DATAOBJ *EvalBinary(DATAOBJ *o1, word operator, DATAOBJ * o2)
{
    ...
    {
        if (o1->type != o2->type || operator!= T_ASSIGN ||
    ...
    for (ot = optable; ot->op; ot++)
    {
        if (ot->token == tok_op)
        {
            *result = (*op1)->type;
            ...
            if (ret == QOP_LE || ret == QOP_LT || ret == QOP_EQ || ret == QOP_NE || ret == QOP_GE ||
                ret == QOP_GT || ret == QOP_LAND || ret == QOP_LOR || ret == QOP_SLE ||
                ret == QOP_SLT || ret == QOP_SEQ || ret == QOP_SNE || ret == QOP_SGE ||
                ret == QOP_SGE || ret == QOP_SGT)
            {
                *result = &WordType;
            }
...
void bad_conversion(TYPEDESC *t1, TYPEDESC *t2)
{
    ...
    if (t1 && t2)
        error(MEXERR_INVALIDTYPECONV, TypeName(t1, name1), TypeName(t2, name2));
}
...
DATAOBJ *ProcessIndex(DATAOBJ *array, DATAOBJ *index, int fLvalue)
{
    ...
    conpart = (array->type->typeinfo.array.el_type->size * array->type->typeinfo.array.bounds.lo);
...
DATAOBJ *ProcessStruct(DATAOBJ *struc, IDTYPE *pid)
{
    ...
    {
        error(MEXERR_FIELDNOTSTRUCTMEMBER, pid, struc->type->typeinfo.struc.name);
