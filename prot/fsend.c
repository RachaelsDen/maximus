...
static unsigned near XmTxInit(struct _xmstuff *px)
{
    ...
    if (px->protocol == PROTOCOL_XMODEM1K || px->protocol == PROTOCOL_YMODEM ||
    ...
    dlogit(("@XmTxInit - size=%ld, blocksize=%ld, n_blocks=%ld", (long)px->size,
            (long)px->blocksize, (long)px->n_blocks));
...
static unsigned near XmTxHeader(struct _xmstuff *px)
{
    ...
    while (px->n_err < 10 && px->n_can < NUM_CAN && carrier())
    {
        ...
        case XM_ACK: /* processed at end of loop */
            break;

        default:
            dlogit(("@hdr: got junk '%c' (%#02x)", ch == 0 ? ' ' : ch, ch));
        }
    ...
    while (px->last_ack < px->n_blocks && px->n_err < 10 && px->n_can < NUM_CAN && carrier())
    {
        ...
        do
        {
            ...
            case XM_ACK:
                ...
                break;
            default:
                dlogit(("@got junk '%c' (%#02x)", ch == 0 ? ' ' : ch, ch));
            }
