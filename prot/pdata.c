...
void ThruLog(long lTotalBytes)
{
    ...
    logit(log_dl_speed, lCPS, lBytesSent, lCPS * 1000L / (long)baud);
}
...
unsigned XferWinOpen(sword protocol, unsigned doing_dl)
{
    ...
    if (gwXfer)
    {
        ...
        sprintf(temp, doing_dl ? win_download : win_upload, Protocol_Name(protocol, temp2));
              ...
              long last_ack, unsigned n_err, long block)
{
    if (gwXfer)
    {
        ...
        if (flag != XS_SENT && flag != XS_RECV)
        {
            ...
            case XS_NAK:
                if (last_ack)
                    WinPrintf(gwXfer, win_nak_block, last_ack, 10 - n_err);
                break;

            case XS_EOT:
                WinPuts(gwXfer, win_sending_eot);
                break;
            case XS_GOTEOT:
                WinPuts(gwXfer, win_got_eot);
                break;
            }
        }
    ...
    else
    {
        ...
        Lprintf("%s %ld", flag == XS_NAK ? win_nak : flag == XS_SENT ? win_sent : win_eot, block);
...
void ZmStatHdr(int fSend, int iHdr, char *szHdr, unsigned long ulHdr)
{
    ...
    if (iHdr == ZRPOS && ulHdr == 0)
        ...
        WinPrintf(gwXfer, "%s %s%s", fSend ? win_sent : win_got, szHdr, szAt);
