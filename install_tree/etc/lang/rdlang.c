...
void doRead(int fd, void *buf, int len, char *what)
{
...
void doSeek(int fd, long ofs, int mode, char *what)
{
...
void *zalloc(unsigned sz, char *what)
{
    void *p = malloc(sz);
    if (!p)
    {
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        for (i = 0; i < ginf.n_heap; ++i)
        {
            ...
            printf(" %4u %8lu %8u %7u %7u %7u %s\n", i, hd.start_ofs, hd.start_num, hd.ndefs,
                   hd.adefs, hd.hlen, nameheap + hd.heapname);
#if DETAILED
            if (hd.ndefs)
            {
