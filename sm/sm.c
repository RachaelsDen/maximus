...
static HWND near InitListbox(HWND hwnd, LINEINFO *li)
{
    ...
    for (i = 0; i < iMaxLines; i++)
    {
        ...
        WinSendMsg(hwndList, LM_SETITEMHANDLE, MPFROMSHORT(i), MPFROMP(li + i));
    }
                             ...
                             FONTMETRICS *pfm)
{
    ...
    WinPopupMenu(hwnd, hwnd, hwndMenu, SHORT1FROMMP(mp1), SHORT2FROMMP(mp1),
                 liInfo[usId].fOnline == 1 ? IDM_VIEW : IDM_START,
                 PU_NONE | PU_MOUSEBUTTON1 | PU_MOUSEBUTTON2 | PU_KEYBOARD | PU_HCONSTRAIN |
                     PU_VCONSTRAIN | PU_POSITIONONITEM);
}
...
static void near MainMenu(HWND hwnd, USHORT usCmd, LINEINFO *liInfo, HWND hwndList)
{
    ...
    } cmdtable[] = {{IDM_START, MainMenuStart},      {IDM_STOP, MainMenuStop},
                    {IDM_VIEW, MainMenuView},        {IDM_LOG, MainMenuLog},
                    {IDM_MESSAGE, MainMenuMessage},  {IDM_GLOBALSTART, MainMenuGStart},
                    {IDM_GLOBALSTOP, MainMenuGStop}, {IDM_GLOBALMESSAGE, MainMenuGMessage}};
...
static void near CreateStdWindow(void)
{
    ...
    hwndFrame =
        WinCreateStdWindow((HWND)HWND_DESKTOP, (ULONG)0, (PULONG)&flFrameFlags, (PSZ)szClientClass,
...
static int ReadPrm(char *pszMax)
{
    ...
    WinMessageBox(HWND_DESKTOP, HWND_DESKTOP, szMsg, "", 0, MB_OK | MB_ICONEXCLAMATION);
