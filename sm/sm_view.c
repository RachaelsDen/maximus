    ...
    if (pli->avp.state == -1)
    {
        ...
        case 0:
            ...
            case 7:
            {
                ...
                VioScrollDn(pli->usCurRow, pli->usCurCol, pli->usCurRow, VIO_WIDTH - 1, 1, &cell,
                            pli->hvps);
...
MRESULT EXPENTRY ViewProc(HWND hwnd, MSGDEF msg, MPARAM mp1, MPARAM mp2)
{
    ...
    case SM_VIO:
        ...
        ViewVio((LINEINFO *)WinQueryWindowPtr(hwnd, 0), PVOIDFROMMP(mp2), SHORT2FROMMP(mp1));
...
{
    ULONG flFrameFlags = FCF_DLGBORDER | FCF_MINBUTTON | FCF_SHELLPOSITION | FCF_SYSMENU |
                         FCF_TASKLIST | FCF_TITLEBAR | FCF_ICON;
    ...
    if (!pli->hwndFrame)
    {
        pli->hwndFrame = WinCreateStdWindow(HWND_DESKTOP, (ULONG)WS_ANIMATE, (PULONG)&flFrameFlags,
                                            (PSZ)szViewClass, (PSZ)NULL, (ULONG)0L, (HMODULE)0,
