...
int free(void *p) { return HeapFree(hh, 0, p); }
#endif /* NO_RUNTIME */
...
DWORD __stdcall RxThread(LPVOID arg)
{
    ...
    for (;;) /* Repeat forever */
    {
        ...
        if (cChars)
        {
            ...
            if ((rc = ReadFile(hc->h, hc->cqRx.pbTail, cChars, &dwBytesRead, &ol)) != FALSE ||
    ...
    for (;;)
    {
        ...
        if (!rc)
        {
            if (GetLastError() == ERROR_IO_PENDING)
                ...
                PDPrintf("@WaitCommEvent error - rc=%d, error=%d\n", 0, GetLastError());
#endif
        }
...
static BOOL _InitBuffers(HCOMM hc, DWORD dwRxBuf, DWORD dwTxBuf)
{
    ...
    if ((hc->cqTx.pbBuf = malloc(dwRxBuf)) == NULL || (hc->cqRx.pbBuf = malloc(dwTxBuf)) == NULL)
    {
        return FALSE;
    }
...
BOOL COMMAPI ComOpenHandle(HANDLE hfComm, HCOMM *phc, DWORD dwRxBuf, DWORD dwTxBuf)
{
    ...
    if ((hc->hTx = CreateThread(NULL, 0, TxThread, (LPVOID)hc, CREATE_SUSPENDED, &dwJunk)) ==
            NULL ||
        (hc->hRx = CreateThread(NULL, 0, RxThread, (LPVOID)hc, CREATE_SUSPENDED, &dwJunk)) ==
            NULL ||
...
BOOL COMMAPI ComRxWait(HCOMM hc, DWORD dwTimeOut)
{
    ...
    if (QueueEmpty(&hc->cqRx))
    {
        ...
        PDPrintf("rx: wait\n");
#endif
        rc = WaitForSingleObject(hc->hevRxWait, dwTimeOut == -1 ? INFINITE : dwTimeOut);
#ifdef DEBUG_PIPE
...
BOOL COMMAPI ComTxWait(HCOMM hc, DWORD dwTimeOut)
{
    ...
    if (!QueueEmpty(&hc->cqTx))
    {
        ...
        PDPrintf("tx: wait\n");
#endif
        rc = WaitForSingleObject(hc->hevTxWait, dwTimeOut == -1 ? INFINITE : dwTimeOut);
#ifdef DEBUG_PIPE
        PDPrintf("tx:endwait\n");
#endif
    }
...
DWORD COMMAPI ComInCount(HCOMM hc) { return QueueGetSize(&hc->cqRx); }
...
DWORD COMMAPI ComOutSpace(HCOMM hc) { return QueueGetFree(&hc->cqTx); }
...
HANDLE COMMAPI ComGetHandle(HCOMM hc) { return hc->h; }
...
USHORT COMMAPI ComSetDCB(HCOMM hc, LPDCB pdcb) { return SetCommState(hc->h, pdcb); }
