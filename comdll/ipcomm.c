...
struct _hcomm
{
    ...
    commHandle_t handleType; /**< Description of handleType. Stub in case we ever get multiple
                                drivers going. */

    /* Private members for this file below */
    struct sockaddr_in
        *saddr_p;          /**< Address we're bound to and listening on. NULL after carrier. */
...
telnet_read_reread:
    ...
    else
    {
        ...
        for (ch = memchr(buf, '\0', bytesRead); ch && bytesRead;
    ...
    for (iac = memchr(buf, cmd_IAC, bytesRead); bytesRead > 0 && iac && (iac < (buf + bytesRead));
         iac = memchr(iac + 1, cmd_IAC, bytesRead))
    {
        ...
        else
        {
            arg = iac[1];
            if (arg == cmd_IAC ||
                (arg == cmd_EC)) /* Escaped (real) IAC (0xff), or erase char command */
            {
                bytesRead -= 1;
...
void negotiateTelnetOptions(HCOMM hc)
{
    ...
    ch = ComPeek(
        hc); /* Get the ball rolling -- ComPeek() will process/consume remote telnet commands */
...
USHORT COMMAPI ComIsOnline(HCOMM hc)
{
    ...
    if (select(hc->h + 1, &rfds, NULL, NULL, &tv) > 0)
    {
        ...
        if (fd >= 0)
        {
            ...
            close(hc->h);
            if (fork())
                _exit(0); /* _exit -> no atexit cleanups! */ /* This should NOT be changed to exec;
                                                                fd limit - Wes */
...
ComRead_getData:
        ...
        hc->ct.ReadIntervalTimeout)
    {
        ...
        tv.tv_usec =
            (hc->ct.ReadIntervalTimeout * 1000) / 16; /* inter char timeout is in 1/16 of a ms */
        tv.tv_sec = 0;
...
OSCOMMHANDLE COMMAPI ComGetHandle(HCOMM hc) { return hc ? hc->h : -1; }
...
USHORT COMMAPI ComSetDCB(HCOMM hc, LPDCB pdcb) { return hc ? SetCommState(hc->h, pdcb) : FALSE; }
