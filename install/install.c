...
static struct _dllinfo
{
    char *szDllName;
    char *szModName;
} maxdll[] = {{"maxbt32.dll", "maxbt32"},   {"maxdb32.dll", "maxdb32"},
              {"mcp32.dll", "mcp32"},       {"msgapi32.dll", "msgapi32"},
              {"mexvm32.dll", "mexvm32"},   {"maxblast.dll", "maxblast"},
              {"maxblas2.dll", "maxblas2"}, {"maxcomm.dll", "maxcomm"},
              {"maxuapi.dll", "maxuapi"},   {NULL, NULL}};
#endif
...
static struct _xlattab xlat_new_max[] = {
    {"Hlp\\", szHlpRep}, {"Misc\\", szMiscRep}, {"Lang\\", szLangRep}, {NULL, NULL}};
...
static int near InstallCopyOver200(char *szDir, int iInstDos, int iInstOs2)
{
    ...
    if (iInstDos)
    {
        if (ftab[ANSI2BBS_EXE].iType != EXEABSENT)
            if (!InstallCopy200One(szDir, ftab[ANS2BBS_EXE].szName, ftab[ANSI2BBS_EXE].szName))
                return FALSE;

        if (ftab[ANSI2MEC_EXE].iType != EXEABSENT)
            if (!InstallCopy200One(szDir, ftab[ANS2MEC_EXE].szName, ftab[ANSI2MEC_EXE].szName))
                return FALSE;

        if (ftab[EDITCALL_EXE].iType != EXEABSENT)
            if (!InstallCopy200One(szDir, ftab[EDITCAL_EXE].szName, ftab[EDITCALL_EXE].szName))
                return FALSE;
    }
    ...
    if (!InstallCopy200File(szDir, ACCEMP_EXE, ACCEM_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, ANS2BBSP_EXE, ANS2BBS_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, ANS2MECP_EXE, ANS2MEC_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, ANS2BBSP_EXE, ANSI2BBS_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, ANS2MECP_EXE, ANSI2MEC_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, CVTUSRP_EXE, CVTUSR_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, EDITCALP_EXE, EDITCALL_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, EDITCALP_EXE, EDITCAL_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, FBP_EXE, FB_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, MAIDP_EXE, MAID_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, MECCAP_EXE, MECCA_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, MRP_EXE, MR_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, ORACLEP_EXE, ORACLE_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, SCANBLDP_EXE, SCANBLD_EXE))
        return FALSE;
    if (!InstallCopy200File(szDir, SILTP_EXE, SILT_EXE))
        return FALSE;
    ...
    for (pf = fizzes; pf->szName; pf++)
    {
        if (!fexist(pf->szName))
        {
            ...
            if (
                ...
                pf->iOS == FIZ_ALL ||
#endif
                (pf->iOS == FIZ_DOS && fDos) || (pf->iOS == FIZ_OS2 && fOs2))
            {
...
static int near InstallUpgrade(void)
{
    ...
    if (ftab[MAXP_EXE].iType == EXEUNKNOWN || ftab[MAX_EXE].iType == EXEUNKNOWN)
    {
    ...
    if (ftab[MAXP_EXE].iType == EXE300OS2 || ftab[MAX_EXE].iType == EXE300DOS)
    {
        f300Upgrade = TRUE;
    ...
    else
    {
        ...
        fExtractFiles = fCvtCtl = fCvtEvt = fCvtTag = fCvtConfig = fRecompile = TRUE;
