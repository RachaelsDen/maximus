...
int Parse_Menu(FILE *ctlfile, char *name)
{
    ...
    if (do_menus)
    {
        ...
        if ((menufile = sopen(temp, O_CREAT | O_TRUNC | O_WRONLY | O_BINARY, SH_DENYNONE,
                              S_IREAD | S_IWRITE)) == -1)
        {
...
static void near Free_Menu(void) { free(menuheap); }
...
static int near Parse_Option(option opt, char *line, struct _opt *thisopt, int menufile)
{
    ...
    if (opt == display_menu || opt == display_file || opt == xtern_erlvl || opt == xtern_run ||
        opt == xtern_dos || opt == xtern_chain || opt == xtern_concur || opt == link_menu ||
        opt == key_poke || opt == o_if || opt == o_menupath || opt == mex || opt == msg_reply_area)
    {
