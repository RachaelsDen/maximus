...
void dprintf(PIPES *pp, char *fmt, ...)
{
    ...
    DosWrite(pp->hfStdout, sz, strlen(sz), &ulWritten);
}
...
static void ThreadStdin(void *pv)
{
    ...
    for (;;)
    {
        ...
        rc = DosWrite(pp->rpStdin.hpWrite, &ch, 1, &ulWritten);
...
static int CreatePipeThreads(PIPES *pp)
{
    ...
    pp->tStdin = _beginthread(ThreadStdin, abStdinStk, sizeof(abStdinStk), pp);

#ifdef MXP_DEBUG
    dprintf(pp, "create pipe threads 2 rc=%d\r\n", pp->tStdin);
#endif

    pp->tStdout = _beginthread(ThreadStdout, abStdoutStk, sizeof(abStdoutStk), pp);
#ifdef MXP_DEBUG
...
static int SpawnProgram(PIPES *pp)
{
    ...
    rc =
        DosExecPgm(szFail, sizeof(szFail), EXEC_SYNC, pp->szArgString, NULL, &rcd, pp->szArgString);
