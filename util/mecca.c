...
{
    Hello("MECCA", "Maximus Embedded Command Compiler (Advanced)", VERSION, "1990, " THIS_YEAR);
...
void Compile(char *inname, char *outname, int mode, int time_comp)
{
    ...
    if (!mode)
    {
        ...
        printf("Compiling %-*s to %-*s: ", max_len, /*strupr*/ fancy_fn(inname), max_len,
               /*strupr*/ fancy_fn(outname));
    }
    ...
    while (p)
    {
        ...
        else
        {
            ...
            if (verbs[verbnum].fptr)
            {
                ...
                Output_Code(verbnum, *outfile, &lcol);
        }
...
void P_Comment(struct _inf *inf) { *inf->p = NULL; }
...
void P_Menu_Cmd(struct _inf *inf)
{
    ...
    if (p && !isdigit(*p))
    {
        ...
        if (x == silt_table_size)
        {
            ...
            sprintf(p = temp, "%d", (int)silt_table[x].opt);
    }
...
void P_Acsfile(struct _inf *inf) { P_Access(inf, 'f'); }

void P_Acs(struct _inf *inf) { P_Access(inf, 'a'); }
...
void P_Steady(struct _inf *inf) { Put_Colour(*inf->outfile, lcol); }

void P_Dim(struct _inf *inf) { Put_Colour(*inf->outfile, lcol &= ~0x08); }

void P_Bright(struct _inf *inf) { Put_Colour(*inf->outfile, lcol |= 0x08); }
...
void P_Repeat(struct _inf *inf) { Output_Code(inf->verbnum, *inf->outfile, &lcol); }

void P_Repeatseq(struct _inf *inf) { Output_Code(inf->verbnum, *inf->outfile, &lcol); }

void P_Locate(struct _inf *inf) { Output_Code(inf->verbnum, *inf->outfile, &lcol); }
