...
int Parse_Session(FILE *ctlfile)
{
    ...
    while (fgets(line, MAX_LINE, ctlfile) != NULL)
    {
        ...
        else if (eqstri(p, "track"))
        {
            ...
            if (eqstri(p, "base"))
                Make_Filename(prm.track_base, s2) else if (eqstri(p, "exclude"))
                    Make_Filename(prm.track_exclude, s2) else if (eqstri(p, "view"))
                        Make_String(prm.track_privview, s2) else if (eqstri(p, "modify"))
                            Make_String(prm.track_privmod, s2) else Unknown_Ctl(linenum, p);
        }
        else if (eqstri(p, "attach"))
        {
            ...
            if (eqstri(p, "base"))
            {
                Make_Filename(prm.attach_base, s2) Init_Attach_Db(s2);
            }
            else if (eqstri(p, "path"))
            {
                ...
                Make_String(prm.attach_archiver, s2) else Unknown_Ctl(linenum, p);
        }
        ...
        else if ((x = 1, eqstri(p, "uses")) || (x = 2, eqstri(p, "file")))
        {
            ...
            if (x == 2 && eqstri(p, "date"))
            {
                ...
                else if (eqstri(p, "yymmdd") || eqstri(p, "mm/dd/yy") || eqstri(p, "mm-dd-yy") ||
                         eqstri(p, "dd") || eqstri(p, "none"))
                    ;
                else
                    Unknown_Ctl(linenum, p);
            }
            else
            {
                ...
                if (eqstri(p, "welcome"))
                {
                    Make_Filename(prm.welcome, s2)
                        ...
                        if (!prm.rookie) prm.rookie = prm.welcome;
        ...
        else if (eqstri(p, "format"))
        {
            ...
            if (eqstri(p, "time"))
                Make_String(prm.timeformat, s2) else if (eqstri(p, "date")) Make_String(
                    prm.dateformat, s2) else if (eqstri(p, "msgheader"))
                    Make_String(prm.msg_header, s2) else if (eqstri(p, "msgformat")) Make_String(
                        prm.msg_format, s2) else if (eqstri(p, "msgfooter"))
                        Make_String(prm.msg_footer, s2) else if (eqstri(p, "fileheader"))
                            Make_String(prm.file_header, s2) else if (eqstri(p, "fileformat"))
                                Make_String(prm.file_format, s2) else if (eqstri(p, "filefooter"))
                                    Make_String(prm.file_footer, s2) else Unknown_Ctl(linenum, p);
        }
#if 0
