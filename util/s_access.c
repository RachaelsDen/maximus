    ...
    while (fgets(line, MAX_LINE, ctlfile) != NULL)
    {
        ...
        else if (eqstri(keyword, "flags"))
        {
            static struct
            {
                char *kwd;
                dword flag;
            } flwords[] = {{"uploadany", CFLAGA_ULBBSOK},
        ...
        else if (eqstri(keyword, "mailflags") || eqstri(keyword, "mail"))
        {
            static struct
            {
                char *kwd;
                dword flag;
            } flwords[] = {{"showpvt", CFLAGM_PVT},         {"editor", CFLAGM_EDITOR},
                           {"localeditor", CFLAGM_LEDITOR}, {"netfree", CFLAGM_NETFREE},
                           {"msgattrany", CFLAGM_ATTRANY},  {"writerdonly", CFLAGM_RDONLYOK},
                           {"norealname", CFLAGM_NOREALNM}, {NULL}};
...
void Write_Access()
{
    ...
    if ((acsfile = open(temp, O_CREAT | O_TRUNC | O_WRONLY | O_BINARY, S_IREAD | S_IWRITE)) == -1)
    {
