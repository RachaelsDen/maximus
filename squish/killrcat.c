...
#define ISRIGHT(c)                                                                                 \
    (((c) >= (byte)0x40 && (c) <= (byte)0x7e) || ((c) >= (byte)0xa1 && (c) <= (byte)0xfe))

word ischin(byte *buf) { return (ISLEFT(buf[0]) && ISRIGHT(buf[1])); }
...
void VioPuts(char *txt) { (void)VioWrtTTY(txt, (USHORT)strlen(txt), 0); }
