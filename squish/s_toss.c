...
static void near ReportSpeed(time_t secs)
{
    ...
    if (config.flag & FLAG_ONEPASS)
    {
        ...
        (void)printf("Processed %lu messages in %lu seconds (%lu.%lu msgs/sec).\n",
                     (unsigned long)processed, (unsigned long)secs, (unsigned long)processed / secs,
...
static void near Toss_Packet_Sequence(char *path, word tflag)
{
    ...
    S_LogMsg("#Tossing%s msgs from %s%s", (config.flag & FLAG_ONEPASS) ? "/scanning" : "", path,
...
{
    S_LogMsg("* %s, %02d/%02d/%02d, %02d:%02d:%02d, by %s %u.%02d", fancy_fn(name),
             in->pkt.month + 1, in->pkt.day, (in->pkt.year % 100), in->pkt.hour, in->pkt.minute,
             in->pkt.second, ProdCode(in->pkt.product),
             (in->pkt.rev_maj > 10) ? (in->pkt.rev_maj / 10) : (in->pkt.rev_maj), in->pkt.rev_min);

    S_LogMsg("- Orig=%hu:%hu/%hu.%hu, Dest=%hu:%hu/%hu.%hu, Type=%s", in->pkt.orig_zone,
             in->pkt.orig_net, in->pkt.orig_node, in->pkt.orig_point, in->pkt.dest_zone,
             in->pkt.dest_net, in->pkt.dest_node, in->pkt.dest_point,
    ...
    else
    {
        ...
        if (!bad_packet)
        {
            ...
            */
            ...
            Tossing_From(in.pkt.orig_zone, (word)in.pkt.orig_net, (word)in.pkt.orig_node,
                         in.pkt.orig_point);
            ...
            {
                S_LogMsg("!Insecure packet from %hu:%hd/%hd.%hu", in.pkt.orig_zone, in.pkt.orig_net,
                         in.pkt.orig_node, in.pkt.orig_point);
...
static int near TossReadMsgFromPkt(struct _inmsg *in)
{
    ...
    if (ux <= 3 /* 2 - damn d'bridge! */ || *(word *)(char *)(&in->pktprefix) == 0 || ux == 0)
    {
        return -1;
    }
    ...
    {
        S_LogMsg("!Grunged: %s, offset=%lx hex", in->pktname, GetPktOfs() - (long)ux);
    ...
    in->ctrl = MsgCreateCtrlBuf(in->textptr + sizeof(XMSG), &in->txt, &in->length);
...
static void near Copy_To_Header(struct _inmsg *in)
{
    ...
    in->msg.attr = in->pktprefix.attr & (MSGPRIVATE | MSGCRASH | MSGREAD | MSGHOLD | MSGXX2 |
                                         MSGRRQ | MSGCPT | MSGARQ | MSGFILE);

    ASCII_Date_To_Binary(in->msg.__ftsc_date, (union stamp_combo *)&in->msg.date_written);
...
static int near CheckInsecure(struct _cfgarea *ar, struct _inmsg *in)
{
    ...
    if (ar)
    {
        ...
        if ((config.flag & FLAG_SECURE) && !NetmailArea(ar) && !BadmsgsArea(ar) && !DupesArea(ar))
        {
...
static int near TossOneMsg(struct _inmsg *in, int badmsg, word tflag)
{
    ...
    *comment = '\0';
    ...
    {
        ar->statarea.in_msgs++;
        ar->statarea.in_bytes += (dword)strlen(in->msg.from) + (dword)strlen(in->msg.to) +
                                 (dword)strlen(in->msg.subj) + (dword)4 + /* for nul bytes */
    ...
    if ((config.flag & FLAG_ONEPASS) &&
        ...
        !noscan &&
#endif
        msglen > 0 && NeedToEcho(txtp) && ar->scan && !NetmailArea(ar) && !BadmsgsArea(ar) &&
        !DupesArea(ar))
    {
    ...
    if ((config.flag & FLAG_ONEPASS) && (config.flag & FLAG_SAVECTRL) == 0 &&
        (ar->type & MSGTYPE_SQUISH) && !NetmailArea(ar) && !BadmsgsArea(ar) && !DupesArea(ar))
    {
    ...
    else
    {
        ...
        if ((msgh = MsgOpenMsg(sq, MOPEN_CREATE, 0L)) == NULL)
        {
            ...
            {
                S_LogMsg("!Error creating message in area %s! (%d)", ar->name, msgapierr);
        ...
        if (NetmailArea(ar) && (config.flag & FLAG_KILLFWD) && (config.flag & FLAG_FRODO))
        {
            in->msg.attr |= MSGKILL;
        }
    ...
    if ((config.flag & FLAG_ONEPASS) && !NetmailArea(ar) && !BadmsgsArea(ar) && !DupesArea(ar) &&
        (ar->flag & AFLAG_PASSTHRU) == 0)
    {
    ...
    for (; ar; ar = ar->next_type)
    {
        ...
        if (MsgGetNumMsg(harea))
        {
            ...
            for (mn = 1L; mn <= MsgHighMsg(harea); mn++)
            {
                ...
                {
                    if (MsgReadMsg(hmsg, msg, 0L, MsgGetTextLen(hmsg) + 1, buffer + sizeof(XMSG),
            ...
            if (ar->type & MSGTYPE_SDM)
            {
              ...
              msg->orig.point=0;
                        ...
                        *last_comment = '\0';
                        ...
                        Tossing_From(msg->orig.zone, msg->orig.net, msg->orig.node,
                                     msg->orig.point);
