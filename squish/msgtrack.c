...
static void near BounceMessage(struct _feat_netmsg far *pfn)
{
    ...
    char *bounce;
    char trailer[] =
        "\r================= e n d   o f   b o u n c e d   m e s s a g e ================\r\r";
    XMSG msg;
    HMSG hmsg;
    ...
    if ((bounce = malloc(1024)) != NULL)
    {
        sprintf(bounce,
                ...
                Address(&msg.orig), Address(&msg.orig), pfn->pMsg->from, pfn->pMsg->orig.zone,
                pfn->pMsg->orig.net, pfn->pMsg->orig.node, pfn->pMsg->orig.point, pfn->pMsg->to,
                pfn->pMsg->dest.zone, pfn->pMsg->dest.net, pfn->pMsg->dest.node,
                pfn->pMsg->dest.point, pfn->pMsg->subj);
    }
...
word FEATENTRY _export FeatureNetMsg(struct _feat_netmsg far *pfn)
{
    ...
    */
    ...
    if (FeatListed(pfn))
    {
        sprintf(temp, "@Listed dest address %u:%u/%u.%u", pfn->pMsg->dest.zone, pfn->pMsg->dest.net,
    ...
    sprintf(temp, "@UNLISTED dest address %u:%u/%u.%u", pfn->pMsg->dest.zone, pfn->pMsg->dest.net,
            pfn->pMsg->dest.node, pfn->pMsg->dest.point);
