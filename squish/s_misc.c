...
void cdecl _junk_cdecl_proc(void) { /* to get around incompatibility in MS-LINK */ }
...
void Fill_Out_Pkthdr(struct _pkthdr *ph, struct _sblist *us, word them_zone, word them_net,
                     word them_node, word them_point)
{
    struct _nodepwd *np;
    struct _sblist sb;
    time_t gmt;
...
{
    return (stricmp(((struct _cfgarea *)a1)->name, ((struct _cfgarea *)a2)->name));
}
...
void Do_Echotoss(char *etname, void (*ScanCmd)(struct _cfgarea *ar, HAREA opensq), int bad_scan_all,
                 char *scan_after)
{
    FILE *etfile;
    struct _cfgarea *ar;
    ...
    if (etname == NULL)
    {
        ...
        for (ar = SkipFirst(config.area); ar; ar = SkipNext(config.area))
        {
            ...
            if (after && ((mode & MODE_toss) == 0 || (ar->flag & AFLAG_TOSSEDTO)))
            {
                (*ScanCmd)(ar, NULL);
            }
        }
...
int CallExtern(char *origcmd, word archiver)
{
    ...
    if (config.flag2 & FLAG2_SWAP)
    {
        ...
        if (errno)
        {
            ...
            case 2:
                errno = 9999;
                break; /* special "swap file full" condition */
            case 3:
                errno = EBADF;
                break;
            }
...
word InvalidDate(byte *da)
{
    ...
    if (isdigit(*da) || *da == ' ')
    {
        ...
        if (da[11] < '0' || da[11] > '2' || da[12] < '0' || da[12] > '9')
            return TRUE;

        if (da[13] != ':')
            return TRUE;

        if (da[14] < '0' || da[14] > '5' || da[15] < '0' || da[15] > '9')
            return TRUE;

        if (da[16] != ':')
            return TRUE;

        if (da[17] < '0' || da[17] > '5' || da[18] < '0' || da[18] > '9')
            return TRUE;
    ...
    else
    {
        ...
        if (((da[4] < '1' || da[4] > '3') && da[4] != ' ') || da[5] < '0' || da[5] > '9')
            return TRUE;
        ...
        if ((da[14] != ' ' && (da[14] < '0' || da[14] > '2')) || da[15] < '0' || da[15] > '9')
            return TRUE;
