...
#ifndef UNIX
        (void)strupr(line); /* do the whole line when path case doesn't matter; otherwise we do it
                               piecemeal as needed - wes */
#endif
...
struct _cfgarea *Declare_Area(char *path, char *tag, char *nodes, word type, word flag)
{
    ...
    if (exists)
    {
        ...
        {
            S_LogMsg("!Path for area %s redeclared as %s", ar->name, path);
