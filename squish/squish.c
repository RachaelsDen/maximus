...
static void near InitializeConfig(void)
{
    ...
    if (ar.action == ACTION_RESCAN)
    {
        ...
        ParseNN(*ar.toscan, &ar.n.zone, &ar.n.net, &ar.n.node, &ar.n.point, FALSE);
    }
...
static void near SquishSquashCycle(void)
{
    ...
    S_LogMsg("+Begin, " SQNAME " v%s (mem=%ldK - main=%p)", version, (long)coreleft() / 1024L,
             (void *)main);
...
#define ADDED 20L
    ...
    S_LogMsg("+End.  Toss=%ld (%ld/s), sent=%ld (%ld/s), mem=%ldK", nmsg_tossed, nmsg_tossed / now,
             nmsg_sent, nmsg_sent / now, (long)coreleft() / 1024L);
